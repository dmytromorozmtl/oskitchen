...
export type InvoiceAiAccuracyBenchmarkP2_44Result = {
  invoiceCount: number;
...
export function buildInvoiceAiAccuracyBenchmarkCorpusP2_44(): InvoiceScannerAccuracyFixture[] {
...
export function runInvoiceAiAccuracyBenchmarkP2_44(
  fixtures: InvoiceScannerAccuracyFixture[],
): InvoiceAiAccuracyBenchmarkP2_44Result {
  ...
  const lineItemAccuracyPct = Math.round(
    (scores.filter((s) => s.lineItemCountMatch && s.lineItemDescriptionMatchPct >= 85).length /
      invoiceCount) *
      100,
  );
  ...
  const thresholdPct = INVOICE_AI_ACCURACY_BENCHMARK_P2_44_MIN_PCT;
  const passed =
  ...
  return {
    ...
    passed,
    thresholdPct,
    minInvoices: INVOICE_AI_ACCURACY_BENCHMARK_P2_44_INVOICE_COUNT,
  };
}

/** Degrade fixtures to prove the 95% P2-44 gate fails when accuracy drops. */
export function buildDegradedInvoiceBenchmarkP2_44Fixtures(
  fixtures: InvoiceScannerAccuracyFixture[],
  degradeCount = 3,
