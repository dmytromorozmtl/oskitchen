  ...
  if (params.topic === "products/update") {
    ...
    if (variants.length === 0) {
      ...
      for (const v of variants) {
        ...
        const inventoryQty = extractShopifyInventoryQuantityFromVariant(v);
        if (inventoryQty != null) {
          await syncShopifyInventoryFromProductWebhook({
            userId: params.userId,
            connectionId: conn.id,
            externalProductId: pid,
            externalVariantId: vid,
            ...
            console.error("shopify_inbound_inventory_sync_failed", err);
          });
        }
      }
    }
