...
const TRIAL_POS_FEATURES: FeatureKey[] = ["pos_terminal", "pos_registers", "pos_receipts"];

/** Resolve all feature gates in one billing pass (developer flags page). */
export async function resolveAllFeaturesForUser(userId: string): Promise<
  Array<{ key: FeatureKey; allowed: boolean; plan: SubscriptionPlan; reason?: string }>
> {
  if (isBillingBypassed()) {
    const sub = await prisma.subscription.findUnique({ where: { userId } });
    const plan = sub?.plan ?? "STARTER";
    return FEATURE_KEYS.map((key) => ({ key, allowed: true, plan }));
  }
  ...
  return FEATURE_KEYS.map((key) => {
    if (!access.hasAppAccess) {
      return { key, allowed: false, plan: access.plan, reason: "trial_ended" };
    }
    if (access.inLocalTrial && TRIAL_POS_FEATURES.includes(key)) {
      return { key, allowed: true, plan: access.plan };
    }
    if (!planMeetsFeature(access.plan, key)) {
      return { key, allowed: false, plan: access.plan, reason: "plan" };
    }
    return { key, allowed: true, plan: access.plan };
  });
}
