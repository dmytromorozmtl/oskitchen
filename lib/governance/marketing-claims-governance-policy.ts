...
export const MARKETING_CLAIMS_SAFE_QUALIFIER_PATTERN =
  /beta|roadmap|coming soon|partner|partner-gated|partner access|not available|not sold|not integrated|not claimed|not included|requires|placeholder|evaluation|only scope|not live today|do not market|non-production|future |evaluating|limitations:|approved uber|when you have|s:\s*false|p:\s*false|preview|directional|diagnostic|not guaranteed|skipped/i;
...
/** Skip roadmap hits embedded in TS identifiers (PascalCase, camelCase, SCREAMING_SNAKE). */
export function isIdentifierEmbeddedRoadmapMatch(
  text: string,
  idx: number,
  matchLen: number,
): boolean {
  ...
  for (const term of MARKETING_CLAIMS_ROADMAP_TERMS) {
    for (const pattern of term.patterns) {
      ...
      while ((match = re.exec(combinedText)) !== null) {
        ...
        if (isIdentifierEmbeddedRoadmapMatch(combinedText, idx, match[0].length)) {
          continue;
        }
