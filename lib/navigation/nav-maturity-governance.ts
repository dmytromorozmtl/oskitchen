...
export const NAV_MATURITY_RULES: readonly NavMaturityRule[] = [
  ...
  },
  {
    ...
    exposure: "preview",
    matrixRef: "Uber Direct dispatch BETA",
  },
  {
  ...
  {
    prefix: "/dashboard/onboarding-hub",
    exposure: "default",
    matrixRef: "Onboarding Hub — single entry",
  },
  {
    prefix: "/dashboard/launch-wizard",
    exposure: "hidden_default",
    matrixRef: "Launch Wizard — reachable via Onboarding Hub",
  },
  {
...
export function navMaturityBadgeForHref(href: string): string | null {
  const rule = getNavMaturityRule(href);
  if (rule?.exposure === "preview" && /BETA/i.test(rule.matrixRef)) {
    return "Beta";
  }
  if (rule) return navMaturityBadgeLabel(rule.exposure);
...
export function shouldShowNavLinkByMaturity(
  ...
  if (isNavAuditSuppressedHref(href)) return false;
