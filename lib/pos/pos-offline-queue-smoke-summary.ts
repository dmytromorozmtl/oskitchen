...
export function auditPosOfflineQueueSmokeWiring(root: string = process.cwd()): {
  ...
  for (const rel of POS_OFFLINE_QUEUE_ERA95_WIRING_PATHS) {
    ...
    if (rel === "lib/pos/offline-pci-local-encryption.ts") {
      ...
      if (src.includes("btoa(trimmed)")) {
        failures.push("offline-pci-local-encryption.ts insecure noop-v1 btoa fallback present");
      }
      if (!src.includes("OfflinePciEncryptionUnavailableError")) {
        failures.push("offline-pci-local-encryption.ts missing OfflinePciEncryptionUnavailableError");
      }
    }
