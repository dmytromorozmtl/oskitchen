...
async function resolveChannelImportWorkspaceId(input: {
  userId: string;
  connectionId: string | null;
}): Promise<string | null> {
  ...
  return resolveOwnerWorkspaceId(input.userId);
}
...
export async function stageWebhookOrderIngest(input: {
  ...
  const workspaceId = await resolveChannelImportWorkspaceId({
    userId: input.userId,
    connectionId: input.connectionId,
  });
