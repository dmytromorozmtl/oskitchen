...
export type PlaybookTenantScope = PlaybookActorScope & {
  userId: string;
  email: string | null;
};

/** Build playbook scope from workspace RBAC + profile role (never assume isOwner). */
export function createPlaybookActorScope(
