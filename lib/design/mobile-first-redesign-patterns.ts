...
export function createDashboardSwipeHandlers(input: {
  ...
  return {
    touchAction: "pan-y",
    onPointerDown: (event) => {
      // Do not setPointerCapture here — capturing on the whole drawer breaks
      // link clicks and nav group toggles inside the sheet (pointerup retargets
      // away from the child, so click never fires).
