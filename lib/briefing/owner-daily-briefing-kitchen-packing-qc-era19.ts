...
} from "@/lib/briefing/owner-daily-briefing-kitchen-packing-qc-era19-policy";
import {
...
export const OWNER_DAILY_BRIEFING_KITCHEN_PACKING_QC_AGGREGATOR_ERA19_POLICY_ID =
  "era19-owner-daily-briefing-kitchen-packing-qc-aggregator-v1" as const;
...
export const BRIEFING_KITCHEN_PACKING_QC_HREF = buildPackingQcClarityHref();

function draftTile(tile: OwnerDailyBriefingTileDraft): OwnerDailyBriefingTile {
  return enrichBriefingTilesLinks([tile])[0]!;
}

export function shouldBriefingKitchenPackingQcHandoff(input: {
  packingQueueOpen: number;
  showPriorityLane: boolean;
}): boolean {
  return input.packingQueueOpen > 0 && input.showPriorityLane;
}

export function resolveBriefingKitchenPackingQcHref(input: {
  packingQueueOpen: number;
  showPriorityLane: boolean;
}): string {
  return shouldBriefingKitchenPackingQcHandoff(input)
    ? BRIEFING_KITCHEN_PACKING_QC_HREF
    : PACKING_QC_CLARITY_ROUTE;
}

export function resolveBriefingKitchenPackingQcLinkState(input: {
  packingQueueOpen: number;
  showPriorityLane: boolean;
...
export function buildBriefingKitchenPackingQcTile(
  ...
  if (!shouldBriefingKitchenPackingQcHandoff({
  ...
  return draftTile({
    id: BRIEFING_KITCHEN_PACKING_QC_TILE_ID,
    category: "packing",
    label: "Packing QC handoff",
    value: `${input.packingQueueOpen} open`,
    detail: `${allergenNote}${input.packingQueueOpen} pack job(s) while KDS priority lane is active — run the 4-step QC checklist before handoff.`,
    href: BRIEFING_KITCHEN_PACKING_QC_HREF,
    availability: "available",
    tone: summary.allergenPrepCount > 0 ? "attention" : "attention",
    priority: 2,
  });
}

export function enrichBriefingKitchenPackingQcPackTiles(
  tiles: readonly OwnerDailyBriefingTile[],
  input: OwnerDailyBriefingKitchenInput,
): OwnerDailyBriefingTile[] {
  const handoffTile = buildBriefingKitchenPackingQcTile(input);
  const summary = buildBriefingKitchenKdsSummary(input.kdsOrders);

  let next = tiles.filter((tile) => tile.id !== BRIEFING_KITCHEN_PACKING_QC_TILE_ID);
  ...
  if (
    ...
    return next.map((tile) => {
      ...
      return {
        ...tile,
        href: BRIEFING_KITCHEN_PACKING_QC_HREF,
        detail: `${tile.detail} KDS priority active — open QC checklist for allergen-safe handoff.`,
...
export function buildOwnerDailyBriefingKitchenPackingQcActions(
  input: OwnerDailyBriefingKitchenInput,
): OwnerDailyBriefingRankedAction[] {
  const summary = buildBriefingKitchenKdsSummary(input.kdsOrders);
  if (
  ...
  const allergenNote =
    summary.allergenPrepCount > 0
      ? ` Allergy alerts on KDS — verify allergens in QC before labels or handoff.`
      : "";

  return [
    {
      id: "kitchen-packing-handoff",
      title: "Run packing QC checklist",
      reason: `${input.packingQueueOpen} pack job(s) open while KDS priority lane is active — align expo pickup with the 4-step QC flow.${allergenNote}`,
...
export function mergeBriefingKitchenPackingQcActions(
  kitchenActions: readonly OwnerDailyBriefingRankedAction[],
  packingQcActions: readonly OwnerDailyBriefingRankedAction[],
): OwnerDailyBriefingRankedAction[] {
  const withoutDuplicate = kitchenActions.filter(
    (action) => action.id !== "kitchen-packing-handoff",
  );
...
export function briefingKitchenPackingQcPolicySnapshot(): {
  policyId: typeof OWNER_DAILY_BRIEFING_KITCHEN_PACKING_QC_ERA19_POLICY_ID;
  qcHref: typeof BRIEFING_KITCHEN_PACKING_QC_HREF;
  qcAnchor: typeof PACKING_QC_CLARITY_ANCHOR;
} {
  return {
    policyId: OWNER_DAILY_BRIEFING_KITCHEN_PACKING_QC_ERA19_POLICY_ID,
    qcHref: BRIEFING_KITCHEN_PACKING_QC_HREF,
