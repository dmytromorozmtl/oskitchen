/**
 * HTML renderer for commercial gate execution report.
 */
...
export function renderCommercialGateExecutionHtml(summary: CommercialGateExecutionSummary): string {
...
<head>
  <meta charset="utf-8" />
  <title>OS Kitchen Commercial Gate Execution</title>
  <style>
...
<body>
  <h1>Commercial Gate — Step 4 Execution</h1>
  <p class="honesty">Policy ${escapeHtml(summary.policyId)} · Generated ${escapeHtml(summary.generatedAt)}</p>

  <div class="banner ${summary.milestone === "commercial_gate_passed" ? "banner-pass" : "banner-blocked"}">
    <strong>Milestone:</strong> ${escapeHtml(summary.milestone)}<br />
    <strong>Tier 2 execution:</strong> ${escapeHtml(summary.tier2ExecutionMilestone ?? "missing")}<br />
    <strong>P0 / Tier 2:</strong> ${escapeHtml(summary.p0ProofStatus ?? "n/a")} / ${escapeHtml(summary.tier2ProofStatus ?? "n/a")}<br />
    <strong>ICP:</strong> ${summary.icpEnvConfigured ? (summary.icpQualified ? "qualified" : "not qualified") : "not configured"}<br />
    <strong>GO/NO-GO:</strong> ${escapeHtml(summary.goDecision ?? "not evaluated")} (${summary.goBlockerCount} blockers) · integrity ${summary.goIntegrityPassed ? "PASS" : "FAIL"}<br />
    <strong>Commercial inflection:</strong> ${escapeHtml(summary.commercialInflectionMilestone)} · pilot score ${summary.pilotExecutableScore}/100
  </div>
  ...
  <h2>Commercial GO closure phases</h2>
  <table>
