/**
 * HTML renderer for continuous improvement loop execution report.
 */
...
export function renderContinuousImprovementLoopExecutionHtml(
...
<body>
  <h1>Continuous Improvement Loop — Step 12 Execution</h1>
  <p class="honesty">Policy ${escapeHtml(summary.policyId)} · Generated ${escapeHtml(summary.generatedAt)}</p>

  <div class="banner ${summary.milestone === "continuous_improvement_loop_passed" ? "banner-pass" : "banner-blocked"}">
    <strong>Milestone:</strong> ${escapeHtml(summary.milestone)}<br />
    <strong>Product evolution:</strong> ${escapeHtml(summary.productEvolutionExecutionMilestone ?? "missing")}<br />
    <strong>GO:</strong> ${escapeHtml(summary.goDecision ?? "not evaluated")} · Customer: ${escapeHtml(summary.customerName ?? "not recorded")}<br />
    <strong>Tracks:</strong> ${summary.tracksHealthy ? "healthy" : `${summary.overdueCount} overdue`} · Pure ops: ${summary.pureOperationalMode ? "active" : "blocked"}<br />
  ...
  <h2>Recurring tracks (7)</h2>
  <table>
    <thead><tr><th>Status</th><th>Track</th><th>Frequency</th><th>Detail</th></tr></thead>
