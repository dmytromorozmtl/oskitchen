/**
 * HTML renderer for sustained operational excellence execution report.
 */
...
export function renderSustainedOperationalExcellenceExecutionHtml(
  ...
  const cadenceRows = summary.phases
    .map(
      (phase) => `
      <tr>
...
<head>
  <meta charset="utf-8" />
  <title>OS Kitchen Sustained Operational Excellence Execution</title>
  <style>
...
<body>
  <h1>Sustained Operational Excellence — Step 10 Execution</h1>
  <p class="honesty">Policy ${escapeHtml(summary.policyId)} · Generated ${escapeHtml(summary.generatedAt)}</p>

  <div class="banner ${summary.milestone === "sustained_operational_excellence_passed" ? "banner-pass" : "banner-blocked"}">
    <strong>Milestone:</strong> ${escapeHtml(summary.milestone)}<br />
    <strong>Market leader:</strong> ${escapeHtml(summary.marketLeaderExecutionMilestone ?? "missing")}<br />
    <strong>GO:</strong> ${escapeHtml(summary.goDecision ?? "not evaluated")} · Customer: ${escapeHtml(summary.customerName ?? "not recorded")}<br />
    <strong>Cadences A–D:</strong> ${summary.cadencesComplete ? "complete" : "in progress"} · Market leader: ${summary.marketLeaderComplete ? "complete" : "blocked"}<br />
  ...
  <h2>Sustained ops cadences A–D</h2>
  <table>
    <thead><tr><th></th><th>Cadence</th><th>Detail</th></tr></thead>
