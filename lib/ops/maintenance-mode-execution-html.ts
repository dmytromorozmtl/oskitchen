/**
 * HTML renderer for maintenance mode execution report.
 */
...
export function renderMaintenanceModeExecutionHtml(
  ...
  const rhythmRows = summary.rhythms
    .map(
      (rhythm) => `
      <tr>
        <td class="${rhythm.status === "healthy" || rhythm.status === "guidance" ? "status-pass" : "status-blocked"}">${escapeHtml(rhythm.status)}</td>
        <td>${escapeHtml(rhythm.label)}</td>
        <td>${escapeHtml(rhythm.ownerRole)}</td>
        <td>${escapeHtml(rhythm.detail)}</td>
      </tr>`,
    )
    .join("");
...
<body>
  <h1>Maintenance Mode — Step 13 Execution</h1>
  <p class="honesty">Policy ${escapeHtml(summary.policyId)} · Generated ${escapeHtml(summary.generatedAt)}</p>

  <div class="banner ${summary.milestone === "maintenance_mode_passed" ? "banner-pass" : "banner-blocked"}">
    <strong>Milestone:</strong> ${escapeHtml(summary.milestone)}<br />
    <strong>CI loop:</strong> ${escapeHtml(summary.ciLoopExecutionMilestone ?? "missing")}<br />
    <strong>GO:</strong> ${escapeHtml(summary.goDecision ?? "not evaluated")} · Customer: ${escapeHtml(summary.customerName ?? "not recorded")}<br />
    <strong>Rhythms:</strong> ${summary.rhythmsHealthy ? "healthy" : `${summary.overdueCount} overdue`} · Maintenance: ${summary.maintenanceModeActive ? "active" : "blocked"}<br />
  ...
  <h2>Operator rhythms (10)</h2>
  <table>
    <thead><tr><th>Status</th><th>Rhythm</th><th>Owner</th><th>Detail</th></tr></thead>
