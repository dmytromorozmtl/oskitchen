/**
 * HTML renderer for Series A partner expansion execution report.
 */
...
export function renderSeriesAPartnerExpansionExecutionHtml(
  ...
  const trackRows = summary.phases
    .map(
      (phase) => `
      <tr>
...
<body>
  <h1>Series A / Partner Expansion — Step 8 Execution</h1>
  <p class="honesty">Policy ${escapeHtml(summary.policyId)} · Generated ${escapeHtml(summary.generatedAt)}</p>

  <div class="banner ${summary.milestone === "series_a_partner_expansion_passed" ? "banner-pass" : "banner-blocked"}">
    <strong>Milestone:</strong> ${escapeHtml(summary.milestone)}<br />
    <strong>Production GA:</strong> ${escapeHtml(summary.productionGaMilestone ?? "missing")}<br />
    <strong>GO:</strong> ${escapeHtml(summary.goDecision ?? "not evaluated")} · Customer: ${escapeHtml(summary.customerName ?? "not recorded")}<br />
    <strong>Tracks A–D:</strong> ${summary.tracksComplete ? "complete" : "in progress"} · Scale: ${summary.scaleComplete ? "complete" : "blocked"}<br />
  ...
  <h2>Series A tracks A–D</h2>
  <table>
    <thead><tr><th></th><th>Track</th><th>Detail</th></tr></thead>
