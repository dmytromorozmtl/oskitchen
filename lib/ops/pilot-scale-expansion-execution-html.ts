/**
 * HTML renderer for pilot scale expansion execution report.
 */
...
export function renderPilotScaleExpansionExecutionHtml(
  ...
  const scaleRows = summary.scalePhases
    .map(
      (phase) => `
      <tr>
        <td class="${phase.complete ? "status-pass" : "status-blocked"}">${phase.complete ? "✓" : "✗"}</td>
...
<head>
  <meta charset="utf-8" />
  <title>OS Kitchen Pilot Scale Expansion Execution</title>
  <style>
...
<body>
  <h1>Pilot Scale + Expansion — Step 6 Execution</h1>
  <p class="honesty">Policy ${escapeHtml(summary.policyId)} · Generated ${escapeHtml(summary.generatedAt)}</p>

  <div class="banner ${summary.milestone === "pilot_scale_expansion_passed" ? "banner-pass" : "banner-blocked"}">
    <strong>Milestone:</strong> ${escapeHtml(summary.milestone)}<br />
    <strong>Week 1 execution:</strong> ${escapeHtml(summary.week1ExecutionMilestone ?? "missing")}<br />
    <strong>GO:</strong> ${escapeHtml(summary.goDecision ?? "not evaluated")} · Customer: ${escapeHtml(summary.customerName ?? "not recorded")}<br />
    <strong>Week 2–4:</strong> ${summary.weekPhasesComplete ? "complete" : "in progress"} · Scale: ${summary.scaleComplete ? "complete" : "in progress"}<br />
  ...
  <h2>Week 2–4 expansion phases</h2>
  <table>
    <thead><tr><th></th><th>Phase</th><th>Detail</th></tr></thead>
    <tbody>${weekRows}</tbody>
  </table>

  <h2>Scale readiness gates</h2>
  <table>
    <thead><tr><th></th><th>Gate</th><th>Detail</th></tr></thead>
