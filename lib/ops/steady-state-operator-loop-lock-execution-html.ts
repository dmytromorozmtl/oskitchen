/**
 * HTML renderer for steady-state operator loop lock execution report.
 */
...
export function renderSteadyStateOperatorLoopLockExecutionHtml(
  ...
  const trackRows = summary.tracks
    .map(
      (track) => `
      <tr>
        <td class="${track.status === "healthy" ? "status-pass" : "status-blocked"}">${track.status === "healthy" ? "✓" : "✗"}</td>
        <td>${escapeHtml(track.label)}</td>
        <td>${escapeHtml(track.status)}</td>
        <td>${escapeHtml(track.frequency)}</td>
      </tr>`,
    )
    .join("");
...
<head>
  <meta charset="utf-8" />
  <title>OS Kitchen Steady-State Operator Loop Lock</title>
  <style>
...
<body>
  <h1>Steady-State Operator Loop Lock — Step 15</h1>
  <p class="honesty">Policy ${escapeHtml(summary.policyId)} · Generated ${escapeHtml(summary.generatedAt)}</p>

  <div class="banner ${summary.milestone === "steady_state_operator_loop_passed" ? "banner-pass" : "banner-blocked"}">
    <strong>Milestone:</strong> ${escapeHtml(summary.milestone)}<br />
    <strong>Production pilot ready:</strong> ${escapeHtml(summary.productionPilotReadyClosureMilestone ?? "missing")}<br />
    <strong>Post-terminus steady state:</strong> ${escapeHtml(summary.postTerminusSteadyStateMilestone)}<br />
    <strong>GO:</strong> ${escapeHtml(summary.goDecision ?? "not evaluated")} · Customer: ${escapeHtml(summary.customerName ?? "not recorded")}<br />
    <strong>Tracks:</strong> ${summary.tracksHealthy ? "healthy" : `${summary.overdueCount} overdue`} · Report: ${summary.steadyStateReportPresent ? "present" : "missing"}<br />
  ...
  <h2>Steady-state tracks</h2>
  <table>
    <thead><tr><th></th><th>Track</th><th>Status</th><th>Frequency</th></tr></thead>
