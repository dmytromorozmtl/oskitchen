...
import { buildPciSaqAttestation, pciSaqPdfLines } from "@/lib/compliance/pci-dss-saq";
...
export async function runPciDssSaqAttestation(): Promise<{
  ok: boolean;
  quarter: string;
  ...
  const outDir = process.env.PCI_SAQ_OUT_DIR?.trim() || "/tmp/pci-saq-attestation";
  mkdirSync(outDir, { recursive: true });

  const jsonBody = JSON.stringify(att, null, 2);
  const pdfBody = simplePdf(pciSaqPdfLines(att));
  writeFileSync(join(outDir, `saq-${att.quarter}.json`), jsonBody, "utf-8");
  writeFileSync(join(outDir, `saq-${att.quarter}.pdf`), pdfBody, "utf-8");
  ...
  const jsonUploaded = await uploadAuditCsvToS3WithLock({
    key: `${prefix}/pci-saq/${att.quarter}/attestation.json`,
    body: jsonBody,
    contentType: "application/json",
  });
  const pdfUploaded = await uploadAuditCsvToS3WithLock({
    key: `${prefix}/pci-saq/${att.quarter}/attestation.pdf`,
    body: pdfBody,
    contentType: "application/pdf",
  });

  await uploadAuditCsvToS3WithLock({
    key: `${prefix}/pci-saq/${att.quarter}/manifest.json`,
    body: JSON.stringify({
      quarter: att.quarter,
      overallPassed: att.overallPassed,
  ...
  return {
    ok: att.overallPassed && (jsonUploaded || pdfUploaded),
    quarter: att.quarter,
    overallPassed: att.overallPassed,
