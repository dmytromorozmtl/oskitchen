...
export async function applyStorefrontOrderCheckoutCompleted(
  ...
  if (sfo.storefrontId && storefrontOrderPii.customerEmail) {
    const { earnStorefrontLoyaltyPoints } = await import("@/services/storefront/loyalty-service");
    await earnStorefrontLoyaltyPoints({
      storefrontId: sfo.storefrontId,
      customerEmail: storefrontOrderPii.customerEmail,
      orderId: internalOrder.id,
      earnSubtotal: Number(sfo.subtotal),
    }).catch(() => undefined);
  }
}
