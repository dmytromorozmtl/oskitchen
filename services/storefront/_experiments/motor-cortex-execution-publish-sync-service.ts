...
export async function runMotorCortexExecutionPublishSyncCycle(): Promise<{ synced: number }> {
  if (!isMotorCortexExecutionPublishEnabled()) return { synced: 0 };
  ...
  for (const sf of storefronts) {
    ...
    const merged = applyThemeExperimentPoll(json, syncMotorCortexFromCerebellumAndMidbrain);
  ...
  logger.info("motor_cortex_execution_publish_sync_cycle", { synced });
  return { synced };
}
