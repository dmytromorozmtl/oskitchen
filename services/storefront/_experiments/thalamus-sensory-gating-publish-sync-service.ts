...
export async function runThalamusSensoryGatingPublishSyncCycle(): Promise<{ synced: number }> {
  if (!isThalamusSensoryGatingPublishEnabled()) return { synced: 0 };
  ...
  for (const sf of storefronts) {
    ...
    const merged = applyThemeExperimentPoll(json, syncThalamusFromMidbrainAndSpinal);
  ...
  logger.info("thalamus_sensory_gating_publish_sync_cycle", { synced });
  return { synced };
}
