...
import { sealMultiverseTimelineFromEpochFreeze } from "@/lib/storefront/theme-experiment-multiverse-timeline-seal-crdt";
...
export async function runMultiverseBranchMergeSealCrdtSyncCycle(): Promise<{ synced: number }> {
  if (!isMultiverseBranchMergeSealCrdtEnabled()) return { synced: 0 };
  ...
  for (const sf of storefronts) {
    ...
    json = applyThemeExperimentPoll(json, sealMultiverseTimelineFromEpochFreeze);
    const merged = applyThemeExperimentPoll(json, sealMultiverseBranchMergeFromTimeline);
  ...
  logger.info("multiverse_branch_merge_seal_crdt_sync_cycle", { synced });
  return { synced };
}
