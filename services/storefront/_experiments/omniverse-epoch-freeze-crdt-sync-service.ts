...
export async function runOmniverseEpochFreezeCrdtSyncCycle(): Promise<{ synced: number }> {
  if (!isOmniverseEpochFreezeCrdtEnabled()) return { synced: 0 };
  ...
  for (const sf of storefronts) {
    ...
    const merged = applyThemeExperimentPoll(json, freezeOmniverseEpochFromCausalityLock);
  ...
  logger.info("omniverse_epoch_freeze_crdt_sync_cycle", { synced });
  return { synced };
}
