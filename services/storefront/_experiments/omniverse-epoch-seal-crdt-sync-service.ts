...
export async function runOmniverseEpochSealCrdtSyncCycle(): Promise<{ synced: number }> {
  if (!isOmniverseEpochSealCrdtEnabled()) return { synced: 0 };
  ...
  for (const sf of storefronts) {
    ...
    const merged = applyThemeExperimentPoll(json, sealOmniverseEpochFromReconciliation);
  ...
  logger.info("omniverse_epoch_seal_crdt_sync_cycle", { synced });
  return { synced };
}
