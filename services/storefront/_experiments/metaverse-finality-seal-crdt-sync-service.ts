...
export async function runMetaverseFinalitySealCrdtSyncCycle(): Promise<{ synced: number }> {
  if (!isMetaverseFinalitySealCrdtEnabled()) return { synced: 0 };
  ...
  for (const sf of storefronts) {
    ...
    const merged = applyThemeExperimentPoll(json, sealMetaverseFinalityFromEpoch);
  ...
  logger.info("metaverse_finality_seal_crdt_sync_cycle", { synced });
  return { synced };
}
