...
export async function runSpinalCordPublishThrottleSyncCycle(): Promise<{ synced: number }> {
  if (!isSpinalCordPublishThrottleEnabled()) return { synced: 0 };
  ...
  for (const sf of storefronts) {
    ...
    const merged = applyThemeExperimentPoll(json, syncSpinalThrottleFromBrainstem);
  ...
  logger.info("spinal_cord_publish_throttle_sync_cycle", { synced });
  return { synced };
}
