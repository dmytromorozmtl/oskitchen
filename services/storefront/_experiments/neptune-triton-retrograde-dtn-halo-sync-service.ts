...
export async function runNeptuneTritonRetrogradeDtnHaloSyncCycle(): Promise<{ synced: number }> {
  if (!isNeptuneTritonRetrogradeDtnHaloEnabled()) return { synced: 0 };
  ...
  for (const sf of storefronts) {
    ...
    const merged = applyThemeExperimentPoll(json, syncNeptuneHaloFromTritonRetrograde);
  ...
  logger.info("neptune_triton_retrograde_dtn_halo_sync_cycle", { synced });
  return { synced };
}
