...
export async function runPremotorSmaPlanningPublishSyncCycle(): Promise<{ synced: number }> {
  if (!isPremotorSmaPlanningPublishEnabled()) return { synced: 0 };
  ...
  for (const sf of storefronts) {
    ...
    json = applyThemeExperimentPoll(json, syncCerebellumFromBasalGangliaAndMidbrain);
    json = applyThemeExperimentPoll(json, syncMotorCortexFromCerebellumAndMidbrain);
    const merged = applyThemeExperimentPoll(json, syncPremotorSmaFromMotorCortexAndEthics);
  ...
  logger.info("premotor_sma_planning_publish_sync_cycle", { synced });
  return { synced };
}
