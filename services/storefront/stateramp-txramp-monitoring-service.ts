...
export async function runStateRampTxRampMonitoringPack(): Promise<{
  ok: boolean;
  period: string;
  stateRampReady: boolean;
  txRampReady: boolean;
}> {
  if (!isStateRampTxRampEnabled()) {
    return { ok: true, period: "disabled", stateRampReady: false, txRampReady: false };
  }

  const ev = buildStateRampTxRampEvidence();
  const outDir = process.env.STATERAMP_TXRAMP_OUT_DIR?.trim() || "/tmp/stateramp-txramp";
  ...
  const jsonUploaded = await uploadAuditCsvToS3WithLock({
    key: `${prefix}/stateramp-txramp/${ev.period}/monitoring.json`,
    body: jsonBody,
    contentType: "application/json",
  });
  await uploadAuditCsvToS3WithLock({
    key: `${prefix}/stateramp-txramp/${ev.period}/monitoring.pdf`,
    body: simplePdf(stateRampTxRampPdfLines(ev)),
    contentType: "application/pdf",
  });
  await uploadAuditCsvToS3WithLock({
    key: `${prefix}/stateramp-txramp/${ev.period}/manifest.json`,
    body: JSON.stringify({
      period: ev.period,
      stateRampReady: ev.stateRampReady,
      txRampReady: ev.txRampReady,
