...
export async function runIrapEssentialEightMonitoringPack(): Promise<{
  ok: boolean;
  period: string;
  irapReady: boolean;
}> {
  if (!isIrapEssentialEightEnabled()) {
    return { ok: true, period: "disabled", irapReady: false };
  }

  const ev = buildIrapEssentialEightEvidence();
  const outDir = process.env.IRAP_ESSENTIAL8_OUT_DIR?.trim() || "/tmp/irap-e8";
  ...
  const jsonUploaded = await uploadAuditCsvToS3WithLock({
    key: `${prefix}/irap-essential-eight/${ev.period}/monitoring.json`,
    body: jsonBody,
    contentType: "application/json",
  });
  await uploadAuditCsvToS3WithLock({
    key: `${prefix}/irap-essential-eight/${ev.period}/monitoring.pdf`,
    body: simplePdf(irapEssentialEightPdfLines(ev)),
    contentType: "application/pdf",
  });
  await uploadAuditCsvToS3WithLock({
    key: `${prefix}/irap-essential-eight/${ev.period}/manifest.json`,
    body: JSON.stringify({
      period: ev.period,
      irapReady: ev.irapReady,
      essentialEightMaturity: ev.essentialEightMaturity,
      sha256: createHash("sha256").update(jsonBody).digest("hex"),
    }),
    contentType: "application/json",
  });

  return { ok: jsonUploaded, period: ev.period, irapReady: ev.irapReady };
}
