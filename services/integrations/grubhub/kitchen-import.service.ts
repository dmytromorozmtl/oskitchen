...
export type GrubhubKitchenImportResult = {
  imported: boolean;
  orderId?: string;
  duplicate?: boolean;
};

/** Idempotent Grubhub order → kitchen order (KDS ticket) import. */
export async function importGrubhubOrderToKitchen(input: {
  userId: string;
  workspaceId?: string | null;
  connectionId: string;
  normalized: NormalizedKitchenOrder;
  externalOrderRecordId: string;
}): Promise<GrubhubKitchenImportResult> {
  ...
  const order = await persistResolvedOrder(
    { userId: input.userId, workspaceId: input.workspaceId },
    {
      ...
      customerName: input.normalized.customer.name ?? "Grubhub Guest",
      customerEmail: input.normalized.customer.email ?? "grubhub@import.local",
