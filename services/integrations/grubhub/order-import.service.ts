  ...
  for (const raw of rawOrders) {
    ...
    const order = await persistResolvedOrder(
      { userId, workspaceId: connection.workspaceId },
      {
        ...
        customerName: normalized.customer.name ?? "Grubhub Guest",
        customerEmail: normalized.customer.email ?? `grubhub@import.local`,
        ...
        externalOrderId: normalized.externalOrderId,
        sourceMetadataJson: { provider: "grubhub", rawEvent: normalized.raw },
