...
export type WooCommerceKitchenImportResult = {
  imported: boolean;
  orderId?: string;
  duplicate?: boolean;
};

/** Idempotent WooCommerce order → kitchen order (KDS ticket) import. */
export async function importWooCommerceOrderToKitchen(input: {
  userId: string;
  workspaceId?: string | null;
  connectionId: string;
  normalized: NormalizedKitchenOrder;
  externalOrderRecordId: string;
}): Promise<WooCommerceKitchenImportResult> {
  ...
  const order = await persistResolvedOrder(
    { userId: input.userId, workspaceId: input.workspaceId },
    {
      ...
      customerName: input.normalized.customer.name?.trim() || "WooCommerce Guest",
      customerEmail: input.normalized.customer.email ?? "woocommerce@import.local",
