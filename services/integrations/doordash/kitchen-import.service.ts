...
export type DoorDashKitchenImportResult = {
  imported: boolean;
  orderId?: string;
  duplicate?: boolean;
};

/** Idempotent DoorDash order → kitchen order (KDS ticket) import. */
export async function importDoorDashOrderToKitchen(input: {
  userId: string;
  workspaceId?: string | null;
  connectionId: string;
  normalized: NormalizedKitchenOrder;
  externalOrderRecordId: string;
}): Promise<DoorDashKitchenImportResult> {
  ...
  const order = await persistResolvedOrder(
    { userId: input.userId, workspaceId: input.workspaceId },
    {
      ...
      customerName: input.normalized.customer.name ?? "DoorDash Guest",
      customerEmail: input.normalized.customer.email ?? "doordash@import.local",
