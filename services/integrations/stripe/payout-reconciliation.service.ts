...
import { orderListWhereForOwnerAnd, posTransactionListWhereForOwner } from "@/lib/scope/workspace-resource-scope";
...
export async function reconcileStripePayouts(
  ...
  const posTransactions = paidOrderIds.length
    ? await prisma.pOSTransaction.findMany({
        where: {
          AND: [await posTransactionListWhereForOwner(userId), { orderId: { in: paidOrderIds } }],
        },
