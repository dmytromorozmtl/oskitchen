...
export async function createOrders(input: MarketplaceCheckoutInput): Promise<{
  ...
  await auditLog({
    ...
    source: "USER",
    severity: "INFO",
    entity: { type: "MarketplacePurchaseOrder", id: orders[0]?.id ?? "batch", label: "marketplace.po" },
    metadata: {
      operation: "marketplace.checkout.create_orders",
      orderIds: orders.map((order) => order.id),
      orderTotals: orders.map((order) => ({ id: order.id, total: order.total, vendorId: order.vendorId })),
      requiresApproval,
      paymentMethod: input.paymentMethod,
    },
  });
...
export async function processPayment(input: {
  ...
  const paymentIntent = await stripe.paymentIntents.create(
    ...
    stripeIdempotencyRequestOptions(marketplacePaymentIntentKey(order.id)),
  );
  ...
  if (!requiresApproval && input.paymentMethod === "CARD" && orders[0]) {
    ...
    if (paymentIntentId) {
      await auditLog({
        ...
        source: "USER",
        severity: "INFO",
        entity: { type: "MarketplacePurchaseOrder", id: orders[0].id, label: "marketplace.payment" },
        metadata: {
          orderId: orders[0].id,
          paymentIntentId,
          paymentMethod: input.paymentMethod,
