...
> {
  ...
  try {
    const intent = await stripe.paymentIntents.capture(
      paymentIntentId,
      stripeIdempotencyRequestOptions(marketplacePaymentCaptureKey(paymentIntentId)),
    );
  ...
  if (
    ...
    try {
      const transfer = await stripe.transfers.create(
        ...
        stripeIdempotencyRequestOptions(vendorPayoutTransferKey(vendor.id, payoutId)),
      );
