...
export async function requestInstantVendorPayout(
  vendorId: string,
  auditContext?: {
    actorUserId: string;
...
> {
  ...
  const payoutId = buildStablePayoutId(
    "INSTANT",
    vendorId,
...

  {
    try {
      const transfer = await stripe.transfers.create(
        ...
        stripeIdempotencyRequestOptions(vendorInstantPayoutTransferKey(vendorId, payoutId)),
      );
      ...
      const payout = await stripe.payouts.create(
        ...
        },
        {
          ...
          ...stripeIdempotencyRequestOptions(vendorInstantPayoutDebitKey(vendorId, payoutId)),
        },
      );
  ...
  if (auditContext?.actorUserId) {
    await auditLog({
      workspaceId: auditContext.workspaceId ?? undefined,
      actor: {
        userId: auditContext.actorUserId,
        email: auditContext.actorEmail ?? null,
        role: auditContext.actorRole ?? null,
      },
      action: AUDIT_ACTIONS.VENDOR_INSTANT_PAYOUT_REQUESTED,
      category: "BILLING",
      source: "USER",
      severity: "WARNING",
      entity: { type: "VendorPayout", id: payoutId, label: payoutId },
      metadata: {
        vendorId,
        amount: grossAmount,
        feeAmount,
