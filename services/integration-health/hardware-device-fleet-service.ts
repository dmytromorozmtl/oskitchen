...
import { ownerScopedAnd, resolveOwnerScopedWhere } from "@/lib/scope/workspace-resource-scope";
...
export async function loadHardwareDeviceFleetModel(userId: string): Promise<HardwareDeviceFleetModel> {
  const [registerWhere, terminalWhere] = await Promise.all([
    ownerScopedAnd(userId, {}),
