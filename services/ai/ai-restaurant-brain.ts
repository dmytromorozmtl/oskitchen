...
import { buildChannelTodayForecasts } from "@/lib/ai/predictive-daily-briefing-channel-forecast";
...
export async function generateDailyBriefing(workspaceId: string): Promise<DailyBriefing> {
  ...
  const [
    ...
    loadChannelOrderHistory(ownerUserId),
  ]);
  ...
  const channelForecasts = buildChannelTodayForecasts(channelOrders);
  ...
  const channelConfidence =
    channelForecasts.length > 0
      ? channelForecasts.reduce((s, c) => s + c.confidence, 0) / channelForecasts.length
      : 0;
  const blendedConfidence =
    channelForecasts.length > 0
      ? Math.round(((overallConfidence + channelConfidence) / 2) * 100) / 100
      : overallConfidence;
  ...
  return {
    ...
    workspaceId,
    aiAssisted: true,
    overallConfidence: blendedConfidence,
    inventoryAlerts,
    laborInsights,
    menuInsights,
    staffInsights,
...
async function loadChannelOrderHistory(userId: string) {
