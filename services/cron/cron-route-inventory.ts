...
export function partitionCronRouteSlugs(slugs: string[] = listCronRouteSlugsFromDisk()): CronRoutePartition {
  ...
  const experimental = slugs.filter(
    (s) => isExperimentalCronSlug(s) && !stagingUtilitySet.has(s),
  );
  const unknownOnDisk = slugs.filter(
    (s) =>
      !isAllowedProductionCronSlug(s) &&
      !isExperimentalCronSlug(s) &&
      !stagingUtilitySet.has(s),
  );
