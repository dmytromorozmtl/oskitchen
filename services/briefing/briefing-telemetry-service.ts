...
export async function loadBriefingTelemetrySummary(
  ...
  const [views, clicks, feedback, surfaceGroups, hrefGroups, lastEvent] = await Promise.all([
    prisma.briefingTelemetry.count({
      where: { userId, createdAt: { gte: since }, eventName: "briefing_view" },
    }),
    prisma.briefingTelemetry.count({
      where: { userId, createdAt: { gte: since }, eventName: "briefing_click" },
    }),
    prisma.briefingTelemetry.count({
      where: { userId, createdAt: { gte: since }, eventName: "ai_briefing_feedback" },
    }),
