...
export async function loadCommercialPilotOpsStatusModel(): Promise<CommercialPilotOpsStatusModel> {
  ...
  const p0Summary = loadP0StagingProofUnblockSummary(root);
  const tier2Summary = readJsonArtifact<Tier2StagingGoldenPathSummary>(tier2Path);
  const vaultDisk = readJsonArtifact<unknown>(VAULT_READINESS_REPORT_ARTIFACT);
  ...
  return buildCommercialPilotOpsStatusModel({
    ...
    vaultReadiness: {
      artifactPresent: vaultDisk !== null && isVaultReadinessReport(vaultDisk),
      report: vaultReport,
    },
  });
}
