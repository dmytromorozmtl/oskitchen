...
import {
  ...
  SUSTAINED_OPERATIONAL_EXCELLENCE_BRIEFING_ACTION_PRIORITY,
...
describe("owner-daily-briefing-sustained-operational-excellence-era21", () => {
  it("builds ranked action with priority 8 when sustained ops active", () => {
    ...
    const action = buildOwnerDailyBriefingSustainedOperationalExcellenceAction(slice);
    expect(action?.id).toBe("sustained-operational-excellence");
    expect(action?.priority).toBe(SUSTAINED_OPERATIONAL_EXCELLENCE_BRIEFING_ACTION_PRIORITY);
