...
describe("launch-wizard-market-leader-era32", () => {
  it("builds slice when market leader train active with integrity flags", () => {
    const marketLeader = buildMarketLeaderPositioningUiSlice({
      goNoGoSummary: {
      ...
      env: { ...seriesACompleteEnv, MARKET_LEADER_MOAT_DECK_REVIEWED: "1" },
    });
    const slice = buildLaunchWizardMarketLeaderSlice(marketLeader);
    expect(slice).not.toBeNull();
    expect(slice!.progressLabel).toContain("/");
    expect(launchWizardMarketLeaderHref()).toContain("#launch-wizard-market-leader");
  });
});
