...
describe("KDS Playwright E2E (P1-51)", () => {
  ...
  it("registers audit script, npm script, and deploy gate", () => {
    ...
    expect(pkg.scripts?.["test:ci:kds-playwright-e2e"]).toContain(KDS_PLAYWRIGHT_UNIT_TEST);

    const workflow = readFileSync(join(ROOT, KDS_PLAYWRIGHT_CI_WORKFLOW), "utf8");
    expect(workflow).toContain("audit:kds-playwright-e2e");
  });
