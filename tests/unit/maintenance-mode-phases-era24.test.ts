...
describe("maintenance-mode-phases-era24", () => {
  it("defines ten operator rhythms and four guardrails", () => {
    expect(MAINTENANCE_MODE_RHYTHMS).toHaveLength(10);
  ...
  it("surfaces overdue integration rhythm when channel evidence missing", () => {
    const rhythms = buildMaintenanceModeRhythmStatuses({
      ...
      metricsBaseline: fullArtifacts.metricsBaseline,
      p0Staging: {
        ...fullArtifacts.p0Staging,
        children: {
          ...fullArtifacts.p0Staging.children,
          channelLive: {
            ...fullArtifacts.p0Staging.children.channelLive,
            overall: "FAILED",
          },
        },
      },
    ...
    expect(rhythms.find((rhythm) => rhythm.id === "weekly_wed_integration_health")?.status).toBe(
      "overdue",
    );
  });
});

describe("maintenance-mode-ui-era24", () => {
  it("returns null until product evolution ready", () => {
    expect(
