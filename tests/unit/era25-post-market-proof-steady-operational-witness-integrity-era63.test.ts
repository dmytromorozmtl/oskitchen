...
describe("era25-post-market-proof-steady-operational-witness-integrity-era63", () => {
  it("detects witness started without closure capstone governance closed", () => {
    const result = evaluateEra25PostMarketProofSteadyOperationalWitnessIntegrity(process.cwd(), {
      env: { ERA25_POST_MARKET_PROOF_STEADY_OPERATIONAL_WITNESS_ERA25_ATTESTED: "1" },
      goNoGoOverride: honestGo,
      p0ProofStatusOverride: "proof_passed",
    });
    expect(result.integrityPassed).toBe(false);
    expect(result.era25PostMarketProofSteadyOperationalWitnessExecutionStarted).toBe(true);
    expect(
      result.violations.some((row) => row.id === "witness_without_closure_capstone"),
    ).toBe(true);
  });

  it("detects governance reopen env on witness path", () => {
    const result = evaluateEra25PostMarketProofSteadyOperationalWitnessIntegrity(process.cwd(), {
      env: {
    ...
    expect(
      result.violations.some((row) => row.id === "witness_claims_governance_reopen"),
    ).toBe(true);
  });

  it("detects fake witness attestation before prerequisites", () => {
    const result = evaluateEra25PostMarketProofSteadyOperationalWitnessIntegrity(process.cwd(), {
      env: {
        ERA25_POST_MARKET_PROOF_STEADY_OPERATIONAL_WITNESS_ERA25_ATTESTED: "1",
    ...
    expect(
      result.violations.some((row) => row.id === "fake_witness_attestation"),
    ).toBe(true);
  });

  it("passes when witness env absent", () => {
    const result = evaluateEra25PostMarketProofSteadyOperationalWitnessIntegrity(process.cwd(), {
      env: {},
      goNoGoOverride: honestGo,
    });
    expect(result.era25PostMarketProofSteadyOperationalWitnessExecutionStarted).toBe(false);
