...
describe("sustained-operational-excellence-ui-era21", () => {
  ...
  it("visible when market leader complete and sustained ops cadences remain", () => {
    const slice = buildSustainedOperationalExcellenceUiSlice({
      goNoGoSummary: {
        version: "era17-pilot-gono-go-v1",
        runAt: new Date().toISOString(),
        decision: "GO",
      ...
      env: marketLeaderCompleteEnv,
    });
    expect(slice?.blocked).toBe(true);
    expect(slice?.marketLeaderComplete).toBe(true);
    expect(slice?.sustainedOpsMilestone).toBe("cadence_a_daily_operational");
    expect(slice?.postMarketLeaderOrchestratorCommand).toContain(
      "run-sustained-operational-excellence-post-market-leader-orchestrator",
    );
    expect(formatSustainedOperationalExcellenceProgressLabel(slice!)).toContain("Acme Kitchen");
  });
});
