...
describe("era25-post-steady-product-mode-commercial-ops-rhythm-permanence-integrity-era68", () => {
  it("detects rhythm permanence started without steady product mode witness", () => {
    ...
    expect(result.era25PostSteadyProductModeCommercialOpsRhythmPermanenceExecutionStarted).toBe(
      true,
    );
    expect(
      result.violations.some(
        (row) => row.id === "commercial_ops_rhythm_permanence_without_steady_product_mode",
      ),
    ).toBe(true);
  });

  it("detects governance reopen env on rhythm permanence path", () => {
    const result = evaluateEra25PostSteadyProductModeCommercialOpsRhythmPermanenceIntegrity(
      process.cwd(),
      {
        env: {
    ...
    expect(
      result.violations.some(
        (row) => row.id === "commercial_ops_rhythm_permanence_claims_governance_reopen",
      ),
    ).toBe(true);
  });

  it("detects fake rhythm permanence attestation before prerequisites", () => {
    const result = evaluateEra25PostSteadyProductModeCommercialOpsRhythmPermanenceIntegrity(
      process.cwd(),
      {
        env: {
    ...
    expect(
      result.violations.some((row) => row.id === "fake_commercial_ops_rhythm_permanence_attestation"),
    ).toBe(true);
  });

  it("passes when rhythm permanence env absent", () => {
    const result = evaluateEra25PostSteadyProductModeCommercialOpsRhythmPermanenceIntegrity(
      process.cwd(),
