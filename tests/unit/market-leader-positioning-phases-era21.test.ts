...
describe("market-leader-positioning-phases-era21", () => {
  it("requires GO and Series A complete for prerequisites", () => {
    expect(
      resolveMarketLeaderPositioningPrerequisites({
        goDecision: "GO",
  ...
  it("detects Series A complete from artifacts", () => {
    expect(
      resolveSeriesACompleteForMarketLeader({
        ...
        env: fullArtifacts.seriesAEnv,
      }),
    ).toBe(true);
  });

  it("builds four pillars after prerequisites", () => {
    ...
    expect(resolveNextIncompleteMarketLeaderPositioningPhase(phases)?.id).toBe(
      "pillar1_category_narrative",
    );
  });

  it("completes market leader positioning when all pillars pass", () => {
    const prerequisites = resolveMarketLeaderPositioningPrerequisites({
      goDecision: "GO",
      seriesAComplete: true,
    });
    ...
    expect(resolveMarketLeaderPositioningComplete(phases)).toBe(true);
  });
});
