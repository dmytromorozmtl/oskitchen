...
describe("mobile-first redesign policy (DES-25)", () => {
  ...
  it("detects swipe-left without pointer capture (preserves drawer link clicks)", () => {
    let closed = false;
    const handlers = createDashboardSwipeHandlers({
      onSwipeLeft: () => {
        closed = true;
      },
      minDistance: 40,
    ...
    const target = {
      setPointerCapture: () => {
        throw new Error("setPointerCapture must not run on drawer content");
      },
