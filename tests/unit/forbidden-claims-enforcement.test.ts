...
describe("forbidden claims enforcement", () => {
  ...
  it("scanned marketing surfaces qualify roadmap integration terms", () => {
    const sources = readScannedMarketingSources();
    const roadmap = sources.flatMap(({ label, text }) =>
      findRoadmapTermViolations(text, label),
    );
    expect(
      roadmap,
      roadmap.map((v) => `${v.sourceLabel}: ${v.context.slice(0, 120)}`).join("\n"),
    ).toEqual([]);
  });
  ...
  it("CI quality job references forbidden claims gate", () => {
    const ci = readFileSync(join(ROOT, ".github/workflows/ci.yml"), "utf8");
    expect(ci).toContain("gate:forbidden-claims");
  ...
  it("wires forbidden claims enforcement auto-test in CI and package.json", () => {
    ...
    expect(scripts["test:ci:forbidden-claims-enforcement"]).toContain("gate:forbidden-claims");
