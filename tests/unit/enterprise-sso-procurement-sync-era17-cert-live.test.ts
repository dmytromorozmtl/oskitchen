...
import {
  ...
  ENTERPRISE_SSO_PROCUREMENT_SYNC_ERA17_GATE_REVIEW_SECTION,
...
describe("enterprise SSO procurement sync era17 CI certification (live repo)", () => {
  ...
  it("syncs procurement pack FAQ and questionnaire with pilot_foundation SSO", () => {
    ...
    expect(pack.toLowerCase()).toContain("awaiting_idp_login_proof");
    expect(pack).toContain(ENTERPRISE_SSO_PROCUREMENT_SYNC_ERA17_GATE_REVIEW_SECTION);
