...
function main() {
  ...
  logger.cli(`=== Vercel ${vercelTarget} environment push ===\n`);
  logger.cli(`Target: Vercel → Project → Settings → Environment → **${vercelTarget}**\n`);
  ...
  for (const key of ready) {
    logger.cli(`[ready] ${key}`);
  }
  for (const key of missing) {
    logger.cli(`[MISSING] ${key}`);
  }

  if (missing.length > 0) {
    logger.cli("\nFill missing keys in .env.staging.local / .env.local before push.");
    if (missing.includes("UPSTASH_REDIS_REST_TOKEN")) {
      logger.cli("  Upstash: https://console.upstash.com → REST API → copy URL + token");
    }
    if (missing.includes("PLATFORM_IMPERSONATION_TOTP_SECRET")) {
      logger.cli("  TOTP: npm run setup:impersonation-totp");
    }
  }

  if (dryRun || !apply) {
    logger.cli("\nDry-run / manual paste:");
    logger.cli(`  Vercel Dashboard → Settings → Environment Variables → ${vercelTarget}`);
    logger.cli(`  Or: npm run vercel:staging-push -- --apply${production ? " --production" : ""}`);
  ...
  logger.cli("  npm run verify:staging-env");
  logger.cli("  SMOKE_BASE_URL=$NEXT_PUBLIC_APP_URL npm run smoke:golden-path-http");
}
