...
async function main(): Promise<void> {
  ...
  console.log("Storefront deploy diagnosis\n");
  if (loaded.length) console.log(`Env loaded: ${loaded.join(", ")}\n`);

  const db = validateDatabaseUrl();
  console.log(`DATABASE_URL: ${db.ok ? "✓" : "✗"} ${db.detail}\n`);
  ...
  console.log("── Actions ──\n");
  if (blockers > 0) {
    console.log("1. Vercel → Project → Deployments → copy **Production** domain (e.g. xxx.vercel.app)");
    console.log("2. Update URLs:");
    console.log('   STOREFRONT_KNOWN_PRODUCTION_URL=https://<real> npm run storefront:apply-deploy-urls');
    console.log("3. Redeploy if needed\n");
  }
  if (!anyLive) {
    console.log("4. Until a host returns 200 on /s/hello, skip: staging-qa, post-deploy, Lighthouse, redirect smoke");
    console.log("5. Local dev: npm run dev:safe → curl http://localhost:3000/s/hello\n");
  }
  if (!db.ok) {
    console.log("6. DB scripts: never `source .env.production.local` — run via npm (loads .env.local + production.local safely)\n");
  }

  console.log("Doc: docs/artifacts/DEPLOY_404_PLAYBOOK.md");
