...
function main(): void {
  ...
  for (const f of [".env", ".env.local"] as const) {
    ...
    console.log(`${ok ? "✓" : "✗"} ${f} ${ok ? "present" : "missing"}`);
  }
  console.log("");
  ...
  console.log(`DATABASE_URL : ${describe(dbShape)}`);
  console.log(`DIRECT_URL   : ${describe(directShape)}`);
  console.log("");
  ...
  if (directShape.kind === "other") {
    console.log("⚠ DIRECT_URL is not a recognised Supabase pooler host.");
  }
  ...
  if (rateWarn) {
    console.log(`⚠ Rate limit: ${rateWarn}`);
  } else if (process.env.NODE_ENV === "production") {
    console.log("✓ Distributed rate limit adapter configured");
  }

  if (prisma.status === 0) {
    console.log("✓ Prisma schema env OK (`npx prisma validate` succeeded)");
  } else {
    console.log("✗ Prisma validate failed:");
    if (prisma.stdout) console.log(prisma.stdout);
    if (prisma.stderr) console.log(prisma.stderr);
    process.exitCode = 1;
  }
}
