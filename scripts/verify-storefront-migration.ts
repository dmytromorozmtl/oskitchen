...
async function main() {
  ...
  logger.cli("Running: npx prisma validate");
  try {
  ...
  if (directKind === "supabase-direct-style-host" && dbKind === "supabase-pooler-host") {
    logger.cli("Note: DIRECT_URL points at a Supabase-style host while DATABASE_URL looks like a pooler — typical for Prisma.");
  }
  ...
  if (wantsDbCheck) {
    ...
    logger.cli("Running: read-only column probe for first_party_analytics_mode (schema: current_schema()).");
    try {
      ...
      logger.cli("Column probe: OK (first_party_analytics_mode exists).");
    } catch (e) {
    ...
    logger.cli("Skipping column probe (pass --db-check with DATABASE_URL to verify the physical column).");
  }

  if (process.env.VERIFY_STOREFRONT_MIGRATION_DB === "1" && hasEnv("DATABASE_URL")) {
    logger.cli("Running: npx prisma migrate status (read-only)");
    try {
    ...
    logger.cli("Skipping migrate status (set VERIFY_STOREFRONT_MIGRATION_DB=1 with DATABASE_URL to run migrate status).");
  }

  logger.cli("\nNext steps (manual):");
  logger.cli("  npm run db:deploy");
  logger.cli("  npx prisma migrate status");
  logger.cli("  If columns are missing without migrate: npm run db:repair-storefront");
  logger.cli("  Optional column-only probe: npm run verify:storefront-db");
  logger.cli("\nDone.");
}
