...
function printRunbook(): void {
  ...
  logger.cli(`Era 17 Woo proof (${CHANNEL_LIVE_SMOKE_WOO_ERA17_POLICY_ID})\n`);
  logger.cli(`Era 17 Shopify proof (${CHANNEL_LIVE_SMOKE_SHOPIFY_ERA17_POLICY_ID})\n`);
  for (const [index, step] of CHANNEL_LIVE_SMOKE_SHOPIFY_ERA17_CYCLE_RUNBOOK_STEPS.entries()) {
    logger.cli(`${index + 1}. ${step}`);
  }
