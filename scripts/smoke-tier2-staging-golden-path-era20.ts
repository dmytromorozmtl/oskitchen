...
`);
  ...
  if (hasFlag("--checklist-only")) {
    ...
    console.log(`Playbook: ${TIER2_STAGING_GOLDEN_PATH_ERA20_PLAYBOOK_DOC}\n`);
    console.log("Child smokes (after P0 PASS):");
    for (const script of TIER2_STAGING_GOLDEN_PATH_ERA20_CHILD_SMOKES) {
      console.log(`  npm run ${script}`);
    }
    console.log("\nManual phases:");
    console.log("  TIER2_CHANNEL_WEBHOOK_MANUAL=PASSED  — Woo/Shopify order in Order Hub");
    console.log("  TIER2_KDS_BUMP_MANUAL=PASSED         — KDS bump on staging");
    console.log("  TIER2_PACKING_COMPLETE_MANUAL=PASSED — Packing terminal state");
    console.log("  GITHUB_KDS_STAGING_RUN_URL + OUTCOME — Playwright KDS green URL\n");
    process.exit(0);
  }
