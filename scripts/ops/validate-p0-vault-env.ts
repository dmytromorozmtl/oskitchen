...
function main() {
  ...
  for (const phase of result.phases) {
    ...
    for (const key of phase.present) console.log(`  ✓ ${key}`);
    for (const key of phase.missing) console.log(`  ✗ ${key}`);
    console.log("");
  }

  if (result.missing.length) {
    console.log("Child smoke mapping:");
    for (const entry of P0_STAGING_PROOF_UNBLOCK_ERA17_ENV_VAR_CATALOG) {
      if (!result.missing.includes(entry.key)) continue;
      console.log(`  ${entry.key} → ${entry.childSmokes.join(", ")}`);
    }
    console.log(`\nNext: ${P0_STAGING_PROOF_UNBLOCK_ERA17_OPS_CHECKLIST_DOC}`);
    console.log("      docs/p0-ops-vault-execution-playbook-2026-05-28.md");
    console.log("Then: npm run smoke:p0-staging-proof-unblock");
    process.exit(1);
  }

  console.log("All 11 P0 vars present.");
  console.log("Run: npm run smoke:p0-staging-proof-unblock");
}
