...
function main() {
  ...
  logger.cli(`P0 gate: ${result.p0ProofStatus ?? "missing artifact"}`);
  logger.cli(`Tier 2 artifact: ${result.tier2ProofStatus ?? "no artifact"}`);
  logger.cli(`Tier 2 milestone: ${result.tier2Milestone}\n`);

  if (!result.p0GatePassed) {
    logger.cli("Blocked — configure P0 ops vault first:");
    logger.cli("  npm run ops:run-p0-vault-day0-orchestrator -- --write");
    logger.cli("  npm run ops:validate-p0-vault-env\n");
    process.exit(2);
  }

  for (const phase of result.phases) {
    logger.cli(`${phase.complete ? "✓" : "○"} ${phase.label}`);
    logger.cli(`  ${phase.detail}\n`);
  }

  logger.cli(`Env vars present: ${result.present.length}/${TIER2_GOLDEN_PATH_ALL_TRACKED_ENV_KEYS.length}`);
  if (result.missing.length > 0) {
    logger.cli(`Missing: ${result.missing.join(", ")}`);
  }
  logger.cli("\nOrchestrator: npm run ops:run-tier2-golden-path-post-p0-orchestrator -- --write");
  logger.cli("Smoke: npm run smoke:tier2-staging-golden-path\n");
  process.exit(0);
}
