...
function main() {
  ...
  console.log("=== Step 0: Day 1 complete ===\n");
  if (loaded.length) console.log(`Env files: ${loaded.join(", ")}\n`);

  const env = checkEnvForStep(0);
  for (const key of env.missing) console.log(`MISSING ${key}`);
  if (!env.ok) {
  ...
  if (!process.argv.includes("--skip-preflight")) {
    console.log("Running beta:preflight...\n");
  ...
  if (process.argv.includes("--dry-env")) {
    console.log("Dry env check complete.");
    return;
  }

  if (process.argv.includes("--with-staging-prep")) {
    console.log("Running staging prep (requires DATABASE_URL on this host)...\n");
  ...
  if (!process.env.SMOKE_SESSION_COOKIE?.trim()) {
    console.log("Generating SMOKE_SESSION_COOKIE via Playwright auth.setup...");
    ...
    if (line) {
      process.env.SMOKE_SESSION_COOKIE = line
        ...
        .replace(/'$/, "");
      console.log("OK SMOKE_SESSION_COOKIE from auth state\n");
    }
  }
  ...
  console.log("\nStep 0 COMPLETE");
  console.log("  Next: npm run beta:go-live -- --emails=chef1@,chef2@,chef3@");
  console.log("  Or:   npm run beta:program -- --step=1");
}
