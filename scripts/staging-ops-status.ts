...
async function main() {
  ...
  logger.cli("=== Staging ops status ===\n");
  logger.cli(`Pilot queue: ${done}/${resolved.length} (${pct}%)`);
  logger.cli(`Next blocker: ${next?.id ?? "none"}\n`);
  ...
  for (const { key, required, validate } of KEYS) {
    ...
    logger.cli(`${tag.padEnd(5)} ${key}${hint}`);
  }
  ...
  if (!isValidUpstashUrl(process.env.UPSTASH_REDIS_REST_URL ?? "")) {
    ...
    if (paste.state === "template") {
      logger.cli("      Replace example URL + paste TOKEN from console, then: npm run pilot:upstash:gate");
    }
  }
  ...
  logger.cli("\n--- DO THIS NOW ---\n");
  if (next?.id === "upstash") {
    logger.cli("  1. Edit .env.upstash.paste.local (Upstash Console → REST API)");
    logger.cli("  2. npm run pilot:upstash:gate");
    logger.cli("  3. npm run vercel:staging-push -- --apply && redeploy");
  } else if (next?.id === "deploy-live") {
    logger.cli("  1. Vercel → Redeploy staging");
    logger.cli("  2. npm run pilot:deploy:gate -- --url=https://NEW-PREVIEW.vercel.app");
  } else {
    logger.cli("  npm run pilot:100-next");
  }
  ...
  try {
    ...
    logger.cli("Guide:  docs/generated/NEXT_STEP_INSTRUCTIONS.md");
  } catch {
    /* ignore */
  }
