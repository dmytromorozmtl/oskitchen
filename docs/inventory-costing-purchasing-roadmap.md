...
| ingredient depletion on sale | POS applies recipe depletion when product has active recipe; otherwise impact stays `PENDING_CONFIGURATION` | storefront/modifier/sub-recipe expansion; operator diagnostics UI | depletion event/state rows where needed | extend `pos-recipe-depletion` to other channels | pending vs applied impact visibility | `production.manage` (inventory mutations) | `inventory.pos_depletion_applied`, count completion audit | unit tests for math + POS depletion | sales deplete inventory when recipes configured |
...
| stock counts, adjustments, waste | count completion applies counted qty to `currentStock`; waste logging RBAC via `production.manage` | waste should decrement stock; dedicated `inventory.count` keys | count cycle and adjustment reason structures | waste stock linkage | count cycle UX and waste quick-actions | `production.manage` (interim) | `inventory.count_completed` | count completion + depletion unit tests | operators can count and reconcile stock |
