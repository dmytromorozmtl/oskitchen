# OS Kitchen — Outreach Email Templates

**Version:** 1.0 · **Date:** 24 May 2026  
**Related:** [pilot-program.md](../pilot-program.md) · [sales-deck.md](../sales-deck.md) · [GTM_SALES_PLAYBOOK.md](../GTM_SALES_PLAYBOOK.md)

---

## Template 1: Cold Outreach (Meal Prep)

**Subject:** Quick question about your kitchen operations

Hi [Name],

I came across [Company] and noticed you're doing meal prep — impressive menu!

I'm building OS Kitchen, a platform that helps meal prep kitchens manage orders, production, packing, and delivery from one dashboard. No hardware required — runs on tablets you already own.

We're looking for **3 pilot partners**:

- 3 months at **50% off**
- Free white-glove setup (we configure your menu, products, storefront)
- Weekly check-in calls
- Priority feature requests

Would you be open to a **10-minute call** this week to see if it's a fit?

Best,  
[Your name]  
https://os-kitchen.com/demo

---

## Template 2: Cold Outreach (Ghost Kitchen)

**Subject:** Your ghost kitchen — one dashboard or five?

Hi [Name],

Running multiple brands from one kitchen is complex. Different menus, different packing, different delivery channels.

OS Kitchen gives you **one dashboard for all your virtual brands** — orders, production, packing, and dispatch. Multi-brand command center included.

We're onboarding **3 pilot partners** at 50% off with free setup and weekly support. **10-minute demo?**

[Your name]  
https://os-kitchen.com/demo

---

## Template 3: Cold Outreach (Small Restaurant)

**Subject:** POS + kitchen display — without the hardware lease

Hi [Name],

Most restaurant POS systems lock you into expensive hardware leases. OS Kitchen runs on **tablets you already own** — POS, kitchen display, online ordering, and production board in one web-based platform.

**$29/month** to start. **14-day free trial.** No credit card.

Would a **10-minute walkthrough** be helpful?

[Your name]  
https://os-kitchen.com/demo

---

## Template 4: Follow-up (after demo)

**Subject:** Great chatting — here's your pilot offer

Hi [Name],

Great meeting you today! As discussed, here's the pilot offer:

- **3 months at 50% off** (Starter $15/mo, Pro $40/mo, Team $100/mo)
- Free setup (we'll configure your menu, products, and storefront)
- Weekly 30-min check-in calls
- Priority feature requests — you shape the product

**Next step:** I'll send you the [pilot agreement](pilot-agreement-template.md) and we'll schedule your onboarding session.

Best,  
[Your name]

---

## Template 5: Re-engagement (no response)

**Subject:** Quick follow-up

Hi [Name],

Just circling back on my note about OS Kitchen. I know you're busy running a kitchen — no worries if now isn't the right time.

If you're curious, explore a live demo anytime: **https://os-kitchen.com/demo**

Solution pages by vertical:

- Meal prep: https://os-kitchen.com/solutions/meal-prep
- Ghost kitchen: https://os-kitchen.com/solutions/ghost-kitchens
- Restaurant: https://os-kitchen.com/solutions/restaurants

Best,  
[Your name]

---

## Sending checklist

- [ ] Personalize first line (menu item, city, or recent post)
- [ ] Link to relevant solution page, not just homepage
- [ ] Log in [pilot-tracker.md](pilot-tracker.md)
- [ ] Follow up once after 5 business days (Template 5)
