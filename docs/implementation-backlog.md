...
- Remaining work after current slice: per-report generator page deny cards aligned to `requireReportReadActor`, broader billing UI parity beyond entitlement overrides, and remaining storefront domain/legal routes if any still use legacy-only gates; uploads are largely covered
