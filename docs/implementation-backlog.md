...
- Progress update: initial online-checkout session failures now preserve the already-written `StorefrontOrder` plus internal `Order` pair instead of deleting them, the persisted storefront payment state is explicitly marked `FAILED`, public token-scoped retry now recreates Stripe Checkout only for the same order after a clear checkout-start failure, and focused recovery coverage now proves failed-to-pending-to-paid progression plus a guard that blocks retry while payment is still `PENDING` so KitchenOS does not mint duplicate live checkout sessions; broader duplicate-handling and full E2E failure-path certification still remain.
