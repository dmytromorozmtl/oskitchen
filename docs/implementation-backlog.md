...
- Progress update: checkout/register/shift/refund/void flows plus `app/api/pos/terminal/route.ts` now enforce canonical POS permissions, the main POS shell/registers/shifts/settings entry pages now mirror those permissions, and focused POS RBAC test files/config now cover checkout discount denial, register/shift/refund/void action denials, and terminal route denials across GET/POST/PUT/DELETE with the focused local Vitest path green again; the primary remaining gaps are deeper cashier/manager/owner role matrices, stronger E2E/API lifecycle coverage, and making dependency restoration reproducible enough that the focused runner does not need manual package recovery
