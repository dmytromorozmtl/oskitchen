...
- Progress update: checkout/register/shift/refund/void flows plus `app/api/pos/terminal/route.ts` now enforce canonical POS permissions, the main POS shell/registers/shifts/settings entry pages now mirror those permissions, and focused POS RBAC test files/config have been added; the focused local Vitest path has now been restored to green after lockfile-aligned package repair, so the primary remaining gaps are deeper E2E/API/negative-role coverage plus making dependency restoration reproducible enough that the focused runner does not need manual package recovery
