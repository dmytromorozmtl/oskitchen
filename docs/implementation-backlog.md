...
- Progress update: checkout/register/shift/refund/void flows plus `app/api/pos/terminal/route.ts` now enforce canonical POS permissions, the main POS shell/registers/shifts/settings entry pages now mirror those permissions, and focused POS RBAC test files/config now cover checkout discount denial, register/shift/refund/void action denials, canonical allowed-path shift open/close audit coverage, terminal route denials across GET/POST/PUT/DELETE, terminal token issuance and payment-intent creation allowed-path coverage, malformed terminal JSON request handling, the terminal process/cancel lifecycle edges with canonical allowed-path audit coverage for successful payment capture, page-level deny/allow parity for POS settings and hardware readiness, `pos.access` page parity for handheld and bar-tab surfaces with denied paths proving they do not load tab data, and `actions/pos/tabs.ts` workflow mutations with canonical `pos.access`/`pos.checkout` enforcement plus denied and allowed audit coverage; the primary remaining gaps are deeper cashier/manager/owner role matrices inside broader POS workflows, broader E2E/device lifecycle coverage, and making dependency restoration reproducible enough that the focused runner does not need manual package recovery
