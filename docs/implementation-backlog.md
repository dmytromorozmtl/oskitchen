...
- Progress update: POS, KDS, billing, and integrations mutation paths are on canonical keys; integrations save/disconnect/health/menu-sync actions now share `requireIntegrationsActor` with `INTEGRATION_PERMISSION_DENIED` audits, and billing bridges workspace grants with legacy role strings
...
- Remaining work after current slice: apply the same helper/audit pattern to exports, integrations read-only UI parity, and broader billing UI parity beyond entitlement overrides; uploads are largely covered
