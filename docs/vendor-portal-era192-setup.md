# Vendor Portal 2.0 setup (Era 192)

Era 192 certifies Vendor Portal 2.0 wiring (Round 2): orders inbox, commission invoices, and sales analytics on one supplier hub — with canonical proof via era117 live smoke.
...
| Script | Purpose |
|--------|---------|
| `npm run smoke:vendor-portal-era192` | Full era192 cert + wiring audit |
| `npm run test:ci:vendor-portal-era192` | Era192 + era117 + vendor portal unit tests |
| `npm run test:ci:vendor-portal-era192:cert` | Wiring cert only (CI gate) |
| `npm run smoke:vendor-portal-era117` | Canonical era117 smoke |
...
5. Run `npm run smoke:vendor-portal-era192` — artifact **PASSED**.
...
Summary written to `artifacts/vendor-portal-era192-smoke-summary.json` (gitignored).

See also: [vendor-portal-era117-setup.md](./vendor-portal-era117-setup.md)
