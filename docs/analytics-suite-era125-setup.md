# Analytics Suite smoke setup (Era 125)

Era 125 certifies Analytics Suite wiring: all restaurant metrics on one screen with drill-down links.
...
| Script | Purpose |
|--------|---------|
| `npm run smoke:analytics-suite-era125` | Full era125 cert + wiring audit |
| `npm run test:ci:analytics-suite-era125` | Era125 + analytics suite unit tests |
| `npm run test:ci:analytics-suite-era125:cert` | Wiring cert only (CI gate) |
...
5. Run `npm run smoke:analytics-suite-era125` — artifact **PASSED**.
...
Summary written to `artifacts/analytics-suite-smoke-summary.json` (gitignored).
