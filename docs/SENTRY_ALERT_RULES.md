...
After deploy, trigger a safe failure in staging (`app/api/cron/health-ping/route.ts` — experimental cron, requires `ENABLE_EXPERIMENTAL_CRONS=true`):
...
Full runbook: `docs/sentry-alert-firing-p2-36-runbook.md` (P2-36 — 5-minute alert SLA).
