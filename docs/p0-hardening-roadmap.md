...
- Progress update: the first POS-focused rollout slice is now in place across `actions/pos.ts`, `app/api/pos/terminal/route.ts`, POS page entrypoints, and the central permission registry/mapping files; **Era 3 Cycle 1** adds canonical gates on `actions/costing.ts` (`reports.read.financial`, `workspace.settings`); **Era 3 Cycle 2** adds PO approval gates on `actions/purchasing.ts` (`production.manage` submit, `reports.read.financial` approve/reject)
- Safest implementation plan:
