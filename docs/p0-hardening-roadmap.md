...
- Progress update: POS workspace-role tests, POS subnav parity tests, POS action permission tests, POS tab workflow permission tests, POS settings/hardware page parity tests, POS handheld/tabs page parity tests, POS transactions/receipts/reports page parity tests, and POS terminal route tests now exist in the repo; the focused local/CI execution path has been restored and the POS suite now covers checkout discount denial, register/shift/refund/void action denials, canonical allowed-path shift open/close audit logging, tab create/add/close mutation denials plus allowed-path audit coverage, terminal route denials across GET/POST/PUT/DELETE, terminal token issuance and payment-intent creation allowed-path audit coverage, malformed terminal JSON request handling, the terminal process/cancel lifecycle with canonical allowed-path audit logging for successful payment capture, page-level deny/allow parity for hardware/settings surfaces, and `pos.access`/plan-gated page parity that prevents denied actors from loading ledger/reporting surfaces, while remaining work is deeper cashier/manager/owner matrices inside workflows plus expansion into other P0 surfaces
- Safest implementation plan:
