...
| payment failure recovery | storefront payment core preserves failed attempts with same-token retry, Stripe cancel routes to the order page and marks pending orders failed for retry, workspace audit logs payment failure/retry, Order Hub shows payment failed/pending badges | full E2E failure-path certification still remains | failed-payment and retry metadata | retry and support-state projection | clear retry and failure states | `storefront.manage` for operator tools | `STOREFRONT_PAYMENT_FAILED` / `STOREFRONT_PAYMENT_RETRY_STARTED` workspace audit | payment failure E2E | failed payments have clear retry and support paths |
