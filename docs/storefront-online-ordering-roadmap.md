...
| payment failure recovery | storefront payment core exists and now preserves failed initial card-checkout attempts with same-token retry while blocking retry during still-pending payments | best customer/operator recovery path still needs strengthening | failed-payment and retry metadata | retry and support-state projection | clear retry and failure states | `storefront.manage` for operator tools | payment-failure audit | payment failure E2E | failed payments have clear retry and support paths |
