...
Loyalty and gift cards: POS checkout applies the **kitchen** ledger (`giftCard`, `loyaltyAccount`). Storefront uses **separate** `storefrontGiftCard` / `storefrontLoyalty*` models — codes and points are **not** interchangeable across channels (**dual ledger**). Policies: `era4-cross-channel-rewards-v1`, permanent GTM lock `era6-dual-ledger-gtm-lock-v1`, Era 10 recert `era10-cross-channel-rewards-recert-v1`, Era 14 recert `era14-cross-channel-rewards-recert-v1` (`lib/rewards/cross-channel-rewards-policy.ts`, `lib/rewards/cross-channel-rewards-era10-policy.ts`, `lib/rewards/cross-channel-rewards-era14-policy.ts`).
...
- Unified cross-channel inventory depletion (storefront or online sales automatically reducing on-hand stock). **POS checkout depletes** recipe-linked ingredients when configured; **storefront, public API, and manual orders do not** — permanent GTM lock until a future era ships a certified hook (`era4-pos-only-v1`, `era5-pos-only-gtm-lock-v1`, `era17-pos-only-inventory-lock-v1`, `era17-pilot-inventory-messaging-v1`, `lib/inventory/inventory-depletion-policy.ts`).
