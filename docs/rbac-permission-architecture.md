...
- Workspace gift cards and loyalty program mutations use canonical `giftcards.manage` / `loyalty.manage` via `requireRewardsMutation`; dashboard pages gate issue/edit UI; POS balance lookups allow `pos.checkout` or the respective manage permission (`lib/crm/require-rewards-mutation.ts`); storefront gift card issue (`actions/storefront/gift-cards.ts`) and `/dashboard/storefront/gift-cards` use `giftcards.manage` with storefront resolution via `requireStorefrontGiftCardsPageAccess` (not `storefront.settings`); storefront loyalty admin (`/dashboard/storefront/loyalty`) uses `loyalty.manage` via `requireStorefrontLoyaltyPageAccess` with matching subnav gate (not `storefront.settings`)
