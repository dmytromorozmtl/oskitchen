...
- Status update: POS slice is live on canonical keys for checkout, discount/comp authorization, refunds, voids, register creation, shift open/close, bar-tab workflows, and terminal API routes with focused audit and negative tests; the first KDS slice now registers and enforces `kitchen.view` / `kitchen.bump` on daily KDS actions and the kitchen page entry with denial/bump audit events, while broader billing/integrations/storefront migration and kitchen recall/configure parity remain open
