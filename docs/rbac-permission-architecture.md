...
- Status update: POS slice is now live on canonical keys for checkout, discount/comp authorization, refunds, voids, register creation, shift open/close, bar-tab create/add/close workflows, and terminal API routes; focused POS coverage now also proves canonical allowed-path audit logging for shift open/close, direct shift-service closeout invariants, direct checkout-service register/shift/terminal workflow invariants, a joined checkout lifecycle covering terminal capture, cancel/retry recovery, failed-capture recovery, and intent-creation recovery on the same open shift/register, a higher-fidelity route-to-service terminal lifecycle across `POST`/`DELETE`/`PUT`, direct refund/void service lifecycle behavior, tab workflows, and terminal token/payment intent flows, explicit malformed-JSON handling plus service-failure contracts on the terminal route, direct Stripe Terminal service invariants, and direct cashier/manager/owner outcomes at the `requireMutationPermission()` layer, while broader billing/integrations/storefront/upload migration remains open
