...
- **Parallel CI job (Era 6 + Era 15 recert + Era 16 reporting):** `era6-typecheck-slice-ci-v1` — job `typecheck-slices` runs `npm run typecheck:ci:slices` at **6GB** in parallel with `quality`; **does not replace** `npm run typecheck` → `typecheck:full` (8GB) as canonical gate (`era15-typecheck-slice-recert-v1`).
- **Era 16 slice reporting (Cycle 11):** `era16-typecheck-slice-report-v1` (`lib/ci/typecheck-slice-report.ts`) — `npm run typecheck:report:slices` / `typecheck:ci:slices` runs all four slices and writes `artifacts/typecheck-slice-summary.json` with per-slice PASSED/FAILED (does not stop at first failure); **does not replace** `typecheck:full`; cert `test:ci:typecheck-slice-era16:cert` (chained in `test:ci:typecheck-slice:cert`).
...
- **Era 4 archive policy:** `era4-active-production-only-v1` — 22 production routes under `app/api/cron/`; 121+ experimental handlers in `archive/cron-routes/`
