...
import { DeliveryForm } from "@/components/dashboard/settings/forms/delivery-form";
...
export default async function DeliverySettingsPage() {
  ...
  if (!canUseSettings(actor, "manage_delivery")) notFound();
  const { payload } = await loadSettingsCenter(userId);
  return (
    <div className="space-y-6">
      <SectionHeader sectionKey="delivery" />
      <DeliveryForm initial={payload.delivery} />
    </div>
  );
}
