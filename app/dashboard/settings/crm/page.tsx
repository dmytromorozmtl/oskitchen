...
import { CrmForm } from "@/components/dashboard/settings/forms/crm-form";
...
export default async function CrmSettingsPage() {
  ...
  if (!canUseSettings(actor, "manage_crm")) notFound();
  const { payload } = await loadSettingsCenter(userId);
  return (
    <div className="space-y-6">
      <SectionHeader sectionKey="crm" />
      <CrmForm initial={payload.crm} />
    </div>
  );
}
