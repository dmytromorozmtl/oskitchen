...
import { OperationsForm } from "@/components/dashboard/settings/forms/operations-form";
...
export default async function OperationsSettingsPage() {
  ...
  if (!canUseSettings(actor, "manage_operations")) notFound();
  const { payload } = await loadSettingsCenter(userId);
  return (
    <div className="space-y-6">
      <SectionHeader sectionKey="operations" />
      <OperationsForm initial={payload.operations} />
    </div>
  );
}
