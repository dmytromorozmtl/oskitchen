...
export async function GET(request: Request) {
  ...
  const livePath = "/dashboard/integrations/uber-eats/live";
  ...
  if (!code?.trim() || !state?.trim()) {
    return NextResponse.redirect(
      new URL(`${livePath}?oauth_error=missing_code`, request.url),
    );
  }
  ...
  if (!verified || verified.userId !== session.id) {
    return NextResponse.redirect(
      new URL(`${livePath}?oauth_error=invalid_state`, request.url),
    );
  }

  const result = await completeUberEatsOAuth({
