...
import { verifySkipWebhookSignature } from "@/services/integrations/skip-live-service";
...
export async function POST(request: Request) {
  ...
  const signature =
    ...
    request.headers.get("X-Skip-Signature") ??
    request.headers.get("x-just-eat-signature") ??
    "";
  ...
  return NextResponse.json({
    ...
    importedOrderId: result.importedOrderId,
  });
}
