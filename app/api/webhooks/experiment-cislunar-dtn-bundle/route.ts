...
/**
 * Cislunar BPv7 bundle webhook (GEO relay / Mars edge → earth merge).
 * Auth: Bearer EXPERIMENT_CISLUNAR_DTN_WEBHOOK_SECRET
 */
export async function POST(request: Request) {
  const authError = requireBearerWebhookSecret(request, {
    secret: process.env.EXPERIMENT_CISLUNAR_DTN_WEBHOOK_SECRET,
  });
  if (authError) {
    return authError;
  }
  ...
  const { json: merged, snap } = ingestCislunarBpv7Bundle(sf.themeExperimentJson, {
    sourceNode: parsed.data.sourceNode as CislunarNodeId,
    targetNode: parsed.data.targetNode as CislunarNodeId,
  ...
  logger.info("cislunar_dtn_bundle_webhook", {
    storeSlug: parsed.data.storeSlug,
    bpv7Count: snap.bpv7Count,
