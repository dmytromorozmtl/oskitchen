...
import { exportGlJournalCsv } from "@/services/accounting/gl-depth-accounting-service";
...
export async function GET(request: Request) {
  ...
  const csv = await exportGlJournalCsv(access.actor.dataUserId, p);

  return new NextResponse(csv, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="gl-journal-${p}.csv"`,
    },
  });
}
