...
export async function reorderMenus(orderedIds: string[]) {
  try {
    ...
    const { userId } = await requireTenantActor();
    const menuScope = await menuListWhereForOwner(userId);
    const ownedMenus = await prisma.menu.findMany({
      where: { AND: [menuScope, { id: { in: orderedIds } }] },
      select: { id: true },
    });
    const ownedIds = new Set(ownedMenus.map((menu) => menu.id));
    await prisma.$transaction(
      orderedIds
        .filter((id) => ownedIds.has(id))
        .map((id, index) =>
          prisma.menu.updateMany({
            where: { AND: [menuScope, { id }] },
