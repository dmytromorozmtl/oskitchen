...
import { patchScimUser } from "@/services/scim/scim-provisioning-service";
...
export async function deactivateScimProvisionedUserAction(scimUserId: string) {
  const gate = await requireScimAdminActor("deactivate_scim_user");
  if (!gate.ok) return fail(gate.error);

  const trimmed = scimUserId.trim();
  if (!trimmed) return fail("SCIM user id is required.");

  try {
    const existing = await prisma.scimProvisionedUser.findFirst({
      where: { id: trimmed, workspaceId: gate.actor.workspaceId! },
      select: { role: true },
    });
    if (!existing) return fail("SCIM user not found in this workspace.");
    ...
    revalidatePath(ENTERPRISE_SSO_SCIM_LIVE_PATH);
    return ok({ message: "SCIM user deactivated — workspace access revoked." });
  } catch (error) {
    return fail(error instanceof Error ? error.message : "Failed to deactivate SCIM user.");
  }
}
