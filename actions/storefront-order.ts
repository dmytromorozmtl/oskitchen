...
import { quoteStorefrontLoyaltyRedeemCredit } from "@/lib/loyalty/loyalty-earn-redeem-p2-41-measurement";
import {
...
export async function submitPublicStorefrontOrder(raw: unknown) {
  try {
    ...
    let loyaltyCreditAmount = 0;
    if (body.loyaltyPointsRedeem && body.loyaltyPointsRedeem > 0) {
      const program = await prisma.storefrontLoyaltyProgram.findUnique({
        where: { storefrontId: sf.id, isActive: true },
      });
      if (!program) {
        return { error: "Loyalty program is not active for this storefront." };
      }
      const quote = quoteStorefrontLoyaltyRedeemCredit(body.loyaltyPointsRedeem, {
      ...
      loyaltyCreditAmount = quote.creditAmount;
    ...
    const { createdOrder, storefrontOrder } = await prisma.$transaction(async (tx) => {
          ...
          sourceMetadataJson: (() => {
            const loyaltyMeta =
              loyaltyPointsRedeemed > 0
                ? {
                    storefrontLoyaltyPointsRedeemed: loyaltyPointsRedeemed,
                    storefrontLoyaltyCreditAmount: loyaltyCreditAmount,
                  }
                : {};
            if (wantsOnline && promoId) {
              return {
                pendingStorefrontPromoId: promoId,
                ...loyaltyMeta,
              } as Prisma.InputJsonValue;
            }
            if (Object.keys(loyaltyMeta).length > 0) {
              return loyaltyMeta as Prisma.InputJsonValue;
            }
            return undefined;
          })(),
      ...
      if (loyaltyPointsRedeemed > 0 && body.loyaltyPointsRedeem) {
        ...
        const lr = await redeemLoyaltyPoints(
          {
            storefrontId: sf.id,
            customerEmail: body.customerEmail,
            points: body.loyaltyPointsRedeem,
            orderId: o.orderId,
          },
          tx,
        );
    ...
    if (!wantsOnline) {
      await earnStorefrontLoyaltyPoints({
        storefrontId: sf.id,
        customerEmail: body.customerEmail,
        orderId: createdOrder.orderId,
        earnSubtotal: subtotal,
      }).catch(() => undefined);
    }
