...
export async function updateTableShapeAction(formData: FormData) {
  const gate = await requireRestaurantTableMutation({ operation: 'restaurant_tables.update_shape' });
  if (!gate.ok) return fail(gate.error);
  const { userId } = gate.actor;
  const parsed = updateShapeSchema.safeParse(Object.fromEntries(formData));
  if (!parsed.success) return fail("Invalid data");

  try {
    await tableService.updateTableShape(parsed.data.tableId, userId, parsed.data.shape);
    revalidatePath('/dashboard/tables');
    revalidatePath('/dashboard/floor-plans');
    return ok(undefined);
  } catch {
    return fail("Failed to update table shape");
  }
}
