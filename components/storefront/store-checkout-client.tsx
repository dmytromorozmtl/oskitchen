...
import {
  StorefrontLoyaltyCheckoutPanel,
  type StorefrontLoyaltyCheckoutProgram,
} from "@/components/storefront/storefront-loyalty-checkout-panel";
...
export function StoreCheckoutClient({
  ...
  loyaltyProgram?: StorefrontLoyaltyCheckoutProgram | null;
}) {
  ...
  const [loyaltyPointsRedeem, setLoyaltyPointsRedeem] = React.useState(0);
  const [loyaltyCreditAmount, setLoyaltyCreditAmount] = React.useState(0);
  ...
  const priced = React.useMemo(() => {
    ...
    const taxable = Math.max(0, subtotal - loyaltyCreditAmount);
    const taxResult = computeStorefrontTax({
      subtotal,
      discountAmount: loyaltyCreditAmount,
      deliveryFee,
    ...
    const tip =
      tipPercent != null && tipPercent > 0
        ...
        : 0;
    const total = subtotal - loyaltyCreditAmount + deliveryFee + tax + tip;
    return {
      subtotal,
      rows,
      deliveryFee,
      ...
      tip,
      total,
      taxIncluded: taxResult.taxIncludedInPrices,
      loyaltyCreditAmount,
    };
  }, [
    cart,
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      res = await submitPublicStorefrontOrder({
        ...
        loyaltyPointsRedeem: loyaltyPointsRedeem > 0 ? loyaltyPointsRedeem : undefined,
      });
    } finally {
      setPending(false);
      ...
      <div className="grid gap-8 lg:grid-cols-5">
              ...
              {loyaltyProgram ? (
                <div className="sm:col-span-2">
                  <StorefrontLoyaltyCheckoutPanel
                    ...
                    loyaltyCreditAmount={loyaltyCreditAmount}
                    onRedeemChange={(points, credit) => {
                      setLoyaltyPointsRedeem(points);
                      setLoyaltyCreditAmount(credit);
                    }}
                  />
                </div>
              ) : null}
        ...
        <aside className="lg:col-span-2">
          <div className="sticky top-24 space-y-4 rounded-2xl border border-border/80 bg-card p-5 shadow-sm">
            ...
            {priced.loyaltyCreditAmount > 0 ? (
              <div className="flex justify-between text-sm text-emerald-700 dark:text-emerald-400">
                <span>Loyalty</span>
                <span>-{formatCurrency(priced.loyaltyCreditAmount, currency)}</span>
              </div>
            ) : null}
