import { CheckCircle2, Circle, CircleDot, ClipboardCheck, Lock } from "lucide-react";

import {
  buildPackingQcChecklist,
  packingQcChecklistStepClassName,
  summarizePackingQcChecklist,
  type PackingQcChecklistStepStatus,
} from "@/lib/packing/packing-qc-clarity-era19";
import type { PackingFocusSnapshot } from "@/lib/packing/packing-focus-era18";

function StepIcon({ status }: { status: PackingQcChecklistStepStatus }) {
  if (status === "complete") {
    ...
    >
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          {summary.qcClear ? " · shift QC clear" : null}
        </p>
      </div>
      <ol className="space-y-2">
        {steps.map((step, index) => (
          <li
            key={step.id}
            className={`flex items-start gap-3 rounded-lg border px-3 py-2 ${packingQcChecklistStepClassName(step.status)}`}
            data-testid={`packing-qc-step-${step.id}`}
