...
import {
  ...
  SERIES_A_PARTNER_EXPANSION_PLATFORM_ANCHOR,
} from "@/lib/commercial/series-a-partner-expansion-ui-era21";
import { cn } from "@/lib/utils";

type SeriesAPartnerExpansionPhasesPanelVariant = "dashboard" | "platform" | "compact";

export function SeriesAPartnerExpansionPhasesPanel(props: {
  slice: SeriesAPartnerExpansionUiSlice;
  variant?: SeriesAPartnerExpansionPhasesPanelVariant;
  title?: string;
}) {
  const {
    slice,
    variant = "dashboard",
    title = "Series A / partner expansion — fundraise + channel motion",
  } = props;
  ...
  return (
    <Card
      id={isPlatform ? SERIES_A_PARTNER_EXPANSION_PLATFORM_ANCHOR.slice(1) : undefined}
      className={cn("scroll-mt-24 shadow-sm", cardClass)}
      data-testid="series-a-partner-expansion-phases-panel"
    >
      {!isCompact ? (
        <CardHeader className="pb-2">
          ...
          <CardDescription className={isPlatform ? "text-zinc-400" : undefined}>
            {formatSeriesAPartnerExpansionProgressLabel(slice)} — audited artifacts only; run
            smoke:woo-shopify-live before LIVE partner copy.
          </CardDescription>
        </CardHeader>
      ) : null}
      <CardContent className={cn("space-y-3", isCompact && "pt-4")}>
        {!isCompact ? (
          ...
          <p className="text-sm font-medium">{formatSeriesAPartnerExpansionProgressLabel(slice)}</p>
        )}
        ...
        {!isCompact ? (
          ...
          <Button asChild size="sm" variant="outline" className="rounded-full">
            <Link href={slice.todayHref}>View Series A / partner checklist</Link>
          </Button>
        )}
      </CardContent>
    </Card>
