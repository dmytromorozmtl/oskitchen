...
export function ImplementationPilotReadinessAttentionStrip(props: {
  ...
  variant?: "implementation" | "go-live";
}) {
  const variant = props.variant ?? "implementation";
  ...
  const title =
    variant === "go-live"
      ? "Pilot cutover readiness — resolve before launch"
      : "Pilot readiness — resolve before go-live";
  const testId =
    variant === "go-live"
      ? "go-live-pilot-readiness-attention-strip"
      : "implementation-pilot-readiness-attention-strip";
    ...
    >
      <CardHeader className="pb-2">
        ...
        <CardDescription>
          {summary.hasUrgent
            ? variant === "go-live"
              ? "Channel pilot, SSO, and launch validation gaps prioritized for this launch project."
      ...
      <CardContent className="space-y-2">
        {items.map((item) => (
          <Link
            key={item.id}
            href={item.href}
            data-testid={`${variant === "go-live" ? "go-live" : "implementation"}-pilot-readiness-${item.id}`}
