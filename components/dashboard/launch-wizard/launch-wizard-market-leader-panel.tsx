...
import { LAUNCH_WIZARD_MARKET_LEADER_ANCHOR } from "@/lib/launch-wizard/launch-wizard-market-leader-era32";
import type { LaunchWizardMarketLeaderSlice } from "@/lib/launch-wizard/launch-wizard-market-leader-era32";

export function LaunchWizardMarketLeaderPanel(props: { slice: LaunchWizardMarketLeaderSlice }) {
  const { slice } = props;

  return (
    <Card
      id={LAUNCH_WIZARD_MARKET_LEADER_ANCHOR.slice(1)}
      className="scroll-mt-24 border-rose-200/80 bg-rose-50/20 shadow-sm dark:border-rose-900/50"
      data-testid="launch-wizard-market-leader-panel"
    >
      <CardHeader className="pb-2">
        ...
        <CardDescription>
          Category narrative, competitive moat deck, analyst kit, expansion motion — no overclaim
      ...
      <CardContent className="space-y-3">
        ...
        <p className="font-mono text-[10px] text-muted-foreground">{slice.postSeriesAOrchestratorCommand}</p>
      </CardContent>
    </Card>
  );
}
