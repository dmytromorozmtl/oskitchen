import Link from "next/link";
import { AlertTriangle, ArrowRight, Rocket } from "lucide-react";

import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import {
  pickGoLiveChecklistAttentionItems,
  summarizeGoLiveChecklistFocus,
  type GoLiveChecklistFocusSnapshot,
  type GoLiveChecklistItemFocus,
} from "@/lib/go-live/go-live-focus-era18";

export function GoLiveChecklistAttentionStrip(props: {
  focus: GoLiveChecklistFocusSnapshot;
  items: readonly GoLiveChecklistItemFocus[];
}) {
  const summary = summarizeGoLiveChecklistFocus(props.focus);
  const attentionItems = pickGoLiveChecklistAttentionItems(props.items, props.focus);

  if (attentionItems.length === 0) return null;

  return (
    <Card
      className="border-amber-200/80 bg-amber-50/40 shadow-sm dark:border-amber-900/40 dark:bg-amber-950/20"
      data-testid="go-live-checklist-attention-strip"
    >
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-lg">
          <Rocket className="h-5 w-5 text-muted-foreground" aria-hidden />
          <AlertTriangle className="h-5 w-5 text-amber-600" aria-hidden />
          Launch checklist — finish these first
        </CardTitle>
        <CardDescription>
          {summary.hasUrgent
            ? "Required gaps, blocked items, and overdue tasks prioritized before approvals unlock."
            : `${summary.totalSignals} open checklist signal${summary.totalSignals === 1 ? "" : "s"} on this launch project.`}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-2">
        {attentionItems.map((item) => (
          <Link
            key={item.id}
            href={item.href}
            data-testid={`go-live-checklist-attention-${item.id}`}
            className="flex items-start justify-between gap-3 rounded-xl border border-border/70 bg-background/80 px-3 py-2 text-sm hover:bg-muted/40"
          >
            <div>
              <p className="font-medium">{item.title}</p>
              <p className="text-xs text-muted-foreground">{item.detail}</p>
            </div>
            <ArrowRight className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" aria-hidden />
          </Link>
        ))}
      </CardContent>
    </Card>
  );
}
