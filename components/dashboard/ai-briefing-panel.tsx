...
export function AiBriefingPanel({ briefing }: { briefing: DailyBriefing }) {
  ...
  return (
    <section className="space-y-4" data-testid="ai-briefing-panel">
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <CardDescription className="mt-1">
                AI-assisted suggestions from inventory, labor, menu, profit, and all-channel today forecasts — review before acting.
              </CardDescription>
            </div>
      ...
      {briefing.channelForecasts.length > 0 ? (
        <Card className="border-border/80 shadow-sm" data-testid="predictive-daily-briefing-channels">
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              Predictive daily briefing — all-channel forecasts for today (Toast IQ parity). Directional estimates only.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {briefing.channelForecasts.map((row) => (
              <div
                key={row.channel}
                className="rounded-lg border border-border/70 bg-background/80 p-3 text-sm"
                data-testid={`channel-forecast-${row.channel.toLowerCase()}`}
              >
                ...
                <p className="mt-2 tabular-nums">
                  ~{row.predictedOrders} orders · {fmtMoney(row.predictedRevenue)}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  So far: {row.actualOrdersSoFar} orders · {fmtMoney(row.actualRevenueSoFar)}
                  {row.trendVsLastWeek != null
                    ? ` · WoW ${row.trendVsLastWeek >= 0 ? "+" : ""}${row.trendVsLastWeek}%`
                    : null}
                </p>
          ...
          <Accordion type="multiple" className="w-full">
            ...
            <AccordionItem value="channels">
              <AccordionTrigger>
                ...
                {briefing.channelForecasts.length > 0 ? (
                  <Badge variant="secondary" className="ml-2">
                    {briefing.channelForecasts.length}
                  </Badge>
                ) : null}
              </AccordionTrigger>
              <AccordionContent className="space-y-3">
                {briefing.channelForecasts.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    No channel history yet — connect storefront or marketplace integrations for forecasts.
                  </p>
                ) : (
                  briefing.channelForecasts.map((row) => (
                    <div key={row.channel} className="rounded-md border p-3 text-sm">
                      ...
                      <p className="mt-1 text-xs tabular-nums">
                        Forecast ~{row.predictedOrders} orders · {fmtMoney(row.predictedRevenue)} · actual{" "}
                        {row.actualOrdersSoFar} / {fmtMoney(row.actualRevenueSoFar)}
                      </p>
                    </div>
                  ))
                )}
                <AiBriefingFeedback sectionId="channels" />
              </AccordionContent>
            </AccordionItem>
