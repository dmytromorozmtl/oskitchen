...
import { formatMonth2MarketReadinessProgressLabel } from "@/lib/commercial/month2-market-readiness-ui-era21";
import { MONTH2_MARKET_READINESS_PLATFORM_ANCHOR } from "@/lib/commercial/month2-market-readiness-ui-era21";
import { cn } from "@/lib/utils";

type Month2MarketReadinessPhasesPanelVariant = "dashboard" | "platform" | "compact";

export function Month2MarketReadinessPhasesPanel(props: {
  slice: Month2MarketReadinessUiSlice;
  variant?: Month2MarketReadinessPhasesPanelVariant;
  title?: string;
}) {
  const {
    slice,
    variant = "dashboard",
    title = "Month 2 — market readiness (investor + GTM + publish gate)",
  } = props;
  ...
  return (
    <Card
      id={isPlatform ? MONTH2_MARKET_READINESS_PLATFORM_ANCHOR.slice(1) : undefined}
      className={cn("scroll-mt-24 shadow-sm", cardClass)}
      data-testid="month2-market-readiness-phases-panel"
    >
      {!isCompact ? (
        <CardHeader className="pb-2">
          ...
          <CardDescription className={isPlatform ? "text-zinc-400" : undefined}>
            {formatMonth2MarketReadinessProgressLabel(slice)} — cite real KPIs only; never publish
      ...
      <CardContent className={cn("space-y-3", isCompact && "pt-4")}>
        {!isCompact ? (
          ...
          <p className="text-sm font-medium">{formatMonth2MarketReadinessProgressLabel(slice)}</p>
        )}
                    ...
                    {phase.routes.length > 0 && !phase.complete ? (
                      <div className="mt-2 flex flex-wrap gap-1">
                        {phase.routes.map((route) => (
                          ...
                          >
                            <Link href={route}>{route.replace(/^\/(solutions|dashboard)\//, "")}</Link>
                          </Button>
                        ))}
                      </div>
                    ) : null}
        ...
        {!isCompact ? (
          <>
            ...
            <div className="flex flex-wrap gap-2">
              <Button asChild size="sm" variant="outline" className="rounded-full">
                <Link href={slice.ghostKitchenLandingHref}>Ghost kitchen landing</Link>
              </Button>
              <Button asChild size="sm" variant="outline" className="rounded-full">
                <Link href={slice.mealPrepLandingHref}>Meal prep landing</Link>
              </Button>
              <Button asChild size="sm" variant="outline" className="rounded-full">
          ...
          <Button asChild size="sm" variant="outline" className="rounded-full">
            <Link href={slice.todayHref}>View Month 2 checklist</Link>
          </Button>
        )}
      </CardContent>
    </Card>
