import Link from "next/link";
import { AlertTriangle, ArrowRight } from "lucide-react";

import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { pickOrderHubAttentionItems } from "@/lib/order-hub/order-hub-stuck-state-era18";
import type { OrderHubExactTabCount } from "@/services/order-hub/order-hub-exact-counts-service";

export function OrderHubAttentionStrip(props: {
  mappingBlockedCount: number;
  exactTabCounts: OrderHubExactTabCount[];
}) {
  const items = pickOrderHubAttentionItems({
    mappingBlockedCount: props.mappingBlockedCount,
    exactTabCounts: props.exactTabCounts,
  });

  if (items.length === 0) return null;

  return (
    <Card
      className="border-amber-200/80 bg-amber-50/40 shadow-sm dark:border-amber-900/40 dark:bg-amber-950/20"
      data-testid="order-hub-attention-strip"
    >
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-lg">
          <AlertTriangle className="h-5 w-5 text-amber-600" aria-hidden />
          Stuck orders — fix these first
        </CardTitle>
        <CardDescription>
          Prioritized by impact on kitchen and channel flow. Open a row to act without hunting tabs.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-2">
        {items.map((item) => (
          <Link
            key={item.id}
            href={item.href}
            data-testid={`order-hub-attention-${item.id}`}
            className="flex items-start justify-between gap-3 rounded-xl border border-border/70 bg-background/80 px-3 py-2 text-sm hover:bg-muted/40"
          >
            <div>
              <p className="font-medium">{item.title}</p>
              <p className="text-xs text-muted-foreground">{item.detail}</p>
            </div>
            <ArrowRight className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" aria-hidden />
          </Link>
        ))}
      </CardContent>
    </Card>
  );
}
