...
export function ApiKeysPanel({ keys }: { keys: Row[] }) {
  ...
  const [sandboxKey, setSandboxKey] = React.useState(false);
        ...
        >
          ...
          <label className="flex items-center gap-2 text-sm">
            <Checkbox
              id="sandboxKey"
              checked={sandboxKey}
              onCheckedChange={(checked) => setSandboxKey(checked === true)}
            />
            <span>
              Sandbox key <span className="text-muted-foreground">(kos_test_, 120 req/min)</span>
            </span>
            {sandboxKey ? <input type="hidden" name="sandbox" value="true" /> : null}
          </label>
