...
export function PlatformErrorRecoveryAttentionStrip(props: {
  snapshot: PlatformErrorRecoverySnapshot;
  recentEvents: ObservabilityErrorEvent[];
}) {
  const summary = summarizePlatformErrorRecoverySnapshot(props.snapshot);
  const categoryItems = pickPlatformErrorRecoveryAttentionItems(props.snapshot);
  const eventActions = pickPlatformErrorRecoveryEventNextActions(props.recentEvents);

  if (categoryItems.length === 0 && eventActions.length === 0) return null;

  return (
    <div className="space-y-4" data-testid="platform-error-recovery-attention-strip">
        ...
        <Card className="border-amber-800/60 bg-amber-950/30">
          <CardHeader className="pb-2">
            ...
            <CardDescription className="text-zinc-400">
              {summary.hasUrgent
                ? "Prioritized by customer impact, production uptime, and channel health."
                : `${summary.totalSignals} open signal${summary.totalSignals === 1 ? "" : "s"} across tenants — resolve before the next service window.`}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
            {categoryItems.map((item) => (
              <Link
                key={item.id}
                href={item.href}
                data-testid={`platform-error-recovery-attention-${item.id}`}
      ...
      {eventActions.length > 0 ? (
        <Card className="border-zinc-800 bg-zinc-900/60">
          <CardHeader className="pb-2">
            <CardTitle className="text-base text-white">Recent errors — next action</CardTitle>
            <CardDescription className="text-zinc-500">
              Last 7 days across tenants. Workspace label shown when available.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-2">
            {eventActions.map((action) => (
              <Link
                key={action.id}
                href={action.href}
                data-testid={`platform-error-recovery-event-${action.id}`}
