...
import {
  ...
  summarizePlatformGoLiveFocus,
  type PlatformGoLiveCommandCenterModel,
} from "@/lib/go-live/platform-go-live-focus-era18";

export function PlatformGoLiveAttentionStrip(props: {
  model: PlatformGoLiveCommandCenterModel;
}) {
  const items = pickPlatformGoLiveAttentionItems(props.model.projects);
  const summary = summarizePlatformGoLiveFocus(items);
  ...
  return (
    <Card
      ...
      data-testid="platform-go-live-attention-strip"
    >
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-lg text-amber-100">
          <Rocket className="h-5 w-5 text-amber-400" aria-hidden />
          <AlertTriangle className="h-5 w-5 text-amber-400" aria-hidden />
          Launch queue — handle these first
        </CardTitle>
        <CardDescription className="text-zinc-400">
          {summary.hasUrgent
            ? "Cross-tenant go-live projects with overdue dates, blockers, or low readiness."
            : `${summary.totalSignals} signal${summary.totalSignals === 1 ? "" : "s"} across active launch projects.`}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-2">
        {items.map((item) => (
          <Link
            key={item.id}
            href={item.href}
            data-testid={`platform-go-live-attention-${item.id}`}
