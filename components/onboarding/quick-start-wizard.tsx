...
const STEPS = ["profile", "menu", "integrations", "order"] as const;
...
export function QuickStartWizard({ initialStep }: Props) {
  ...
  const [selectedChannels, setSelectedChannels] = React.useState<QuickStartChannel[]>(["pos"]);
  const [pendingMenuItems, setPendingMenuItems] = React.useState<QuickStartMenuItemInput[]>([]);
  ...
  function goNext() {
    ...
    if (step === "integrations") {
      applyAndGoToOrder(pendingMenuItems);
    }
  }
    ...
    <WizardStepRoot data-testid="quick-start-wizard">
      <WizardStepIntro>
        ...
        <p className="text-muted-foreground">
          Four steps — name your kitchen, seed your menu, pick sales channels, ring up your first
          sale. Connect integrations now or after your first order.
        </p>
        <div
      ...
      {step === "integrations" ? (
        <WizardStepSection data-testid="quick-start-integrations-step">
          <WizardStepHeading>Where do orders come from?</WizardStepHeading>
          <p className={wizardStepDescriptionClass}>
            Pick the channels you plan to use. POS works immediately — connect Shopify, DoorDash,
            and others when you&apos;re ready.
          </p>
          <div className={cn(wizardStepChoiceGridClass, "lg:grid-cols-2")}>
            {QUICK_START_CHANNEL_OPTIONS.map((opt) => {
              const selected =
                selectedChannels.includes(opt.id) ||
                (opt.id !== "all" && selectedChannels.includes("all"));
              return (
                <button
                  key={opt.id}
                  type="button"
                  data-testid={`quick-start-channel-${opt.id}`}
                  onClick={() => toggleChannel(opt.id)}
                  className={cn(
                    wizardStepChoiceCardClass,
                    "text-left",
                    selected && wizardStepChoiceCardSelectedClass,
                  )}
                >
          ...
          <Card className="border-border/80 bg-muted/20">
            <CardContent className="space-y-2 p-4 text-sm">
              <p className="font-medium">Connect integrations (optional)</p>
              <p className="text-muted-foreground">
                Link sales channels now or skip — you can connect anytime from Integration Health.
              </p>
